  ...
  (0,
   [          
     ...
     (else_try),
       ...
       #yifeng(else_try),
        #yifeng (ge,"$auto_besiege_town",1),
         #yifeng(start_encounter, "$auto_besiege_town"),
       (else_try),
         (ge,"$g_camp_mode", 1),
...
# This trigger will check if player's raid has been completed and will lead control to village menu.
  (1,
   [
     ...
     #yifeng (else_try),
      #yifeng  (map_free), #we have been attacked during raid
      #yifeng  (assign,"$g_player_raiding_village",0),
      (else_try),
...
(0.25,
  ...
  (24 * 7,
   [
     ...
     (try_end),
	 (call_script,"script_renew_supply"),
    ]),
		...
		(try_for_range, ":faction_2", kingdoms_begin, kingdoms_end),
			...
			(try_begin),
				...
				(else_try), #Provocation expires
	                ...
	                (faction_get_slot, ":faction_leader", ":faction_1", slot_faction_leader),
...
#			(display_message, "@{s65}"),
  ...
  (24 * 7,
   ...
   (24 * 7,
   [
   	(call_script,"script_tax_rate_event"),
      ...
      (try_for_range, ":center_no", centers_begin, centers_end),
        (try_begin),
          ...
          (try_begin),
            ...
            (try_end),                
          (try_end),  
#add by windmelodie for tax begin
		(val_mul, ":cur_rents",7),
		(val_div, ":cur_rents",5),
		
		(party_get_slot, ":tax_rate", ":center_no", slot_center_tax_rate),#tax_rate = 1...3
		(val_add,":tax_rate",1),
		(val_mul, ":cur_rents",":tax_rate"),
		(val_div, ":cur_rents",3), #rent* 2/3...4/3
#add by windmelodie for tax end	  
		...
		(try_end),		
      (try_end),
#add by windmelodie for training begin
	(try_for_range, ":center_no", castles_begin,castles_end),
		(party_slot_ge,":center_no", slot_castle_training_rate,1),
		(party_get_slot, ":rents", ":center_no", slot_center_accumulated_rents),
		(party_get_slot, ":rate", ":center_no", slot_castle_training_rate),
		(store_div,":rents_minus",":rents",2),
		(val_mul,":rents_minus",":rate"),
		(val_sub,":rents",":rents_minus"),
		(party_set_slot,":center_no", slot_center_accumulated_rents, ":rents"),
		(val_mul,":rents_minus",10),
		(party_upgrade_with_xp, ":center_no", ":rents_minus",0),
	(try_end),
#add by windmelodie for training end
  ...
  (48, 
   [
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
		 ...
		 ## ZZ debug for prisoner in zhai not count.
		 (assign,":continue",1),			
         (party_get_num_prisoner_stacks, ":num_stacks", "p_zhai"),	 
         (try_for_range, ":cur_stack", 0, ":num_stacks"),
                (party_prisoner_stack_get_troop_id , ":prisoner_troop", "p_zhai", ":cur_stack"),
                (ge, ":prisoner_troop", 1),
                (eq, ":prisoner_troop", ":troop_no"),
				#(display_message,"@is in zhai!!!!",0xFF0000),				
				(assign,":continue",0),				
         (try_end),	 
		 (eq,":continue",1),
         ## ZZ debug for prisoner in zhai not count.
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
     ...
     (assign,"$temp_castle_troop",":troop_no"),
	 (store_random_in_range,":num",10,20),	 
     (assign,"$temp_castle_num",":num"), 
	 ## ZZ	 
    ]),
  ...
  # Checking center upgrades #changed by windmelodie for castle
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
	  ...
	  (try_begin),
		(eq,":cur_improvement",slot_center_has_castle),
	    (enable_party, "p_castle_98"),
		(assign,":end",99),
		(try_for_range,":unused",0,":end"),
		    (party_relocate_near_party, "p_castle_98", "p_village_193",5),
			(party_get_current_terrain,":terrain","p_castle_98"),
			(neq,":terrain",rt_mountain),
			(neq,":terrain",rt_deep_ocean),
			(neq,":terrain",rt_water),
			(neq,":terrain",rt_bridge),
			(assign,":end",0),
		(try_end),
		
		#changed by windmelodie 1.56 20150206 begin
		(try_begin),
		...
		(try_end),
		#changed by windmelodie 1.56 end
		
		(call_script, "script_give_center_to_faction_aux", "p_castle_98", "fac_player_supporters_faction"),
		(call_script, "script_give_center_to_lord", "p_castle_98", "trp_player", 0),
		...
		(party_set_slot, "p_castle_98", slot_center_accumulated_rents, 0),
		(party_set_slot, "p_castle_98", slot_center_accumulated_tariffs, 0),
		
	  (else_try),
		(eq,":cur_improvement",slot_center_has_supply_center),
		(party_set_slot, ":center_no", slot_castle_supply, 1),
	  (try_end),
	  #changed by windmelodie 1.56 end
	...
	(try_begin),
			...
			(party_slot_eq, "p_village_193", slot_outpost_notice, 0),
			(neg|party_slot_eq, "p_village_193", slot_town_lord, "trp_player"),
			(dialog_box,"str_village_notice"),
			(party_set_slot, "p_village_193", slot_outpost_notice, 1),
	(try_end),
	#add by windmelodie end
...
#    (ge, "$g_last_rest_center", 0),
 ...
 # Check if a faction is defeated every day#24yifeng
   (6,
   [
...
## ZZ bank interest on deposits begin
  (24 * 7,
   [
    ...
    (try_begin),
       (gt, ":deposit", 0),
	   (val_mul,":deposit","$g_interest_deposit"),
       (store_div,":interest_deposit",":deposit",100),
  ...
  (24 * 7,
   [
    ...
    (try_begin),
       (gt, ":debt", 0),
	   (val_mul,":debt","$g_interest_debt"),
       (store_div,":interest_debt",":debt",100),
  ...
  (24 * 14,
   [
	   (call_script, "script_interest_rate_according_diplomacy"),
           (troop_get_slot,":loanline_addition","trp_player",slot_troop_line_of_credit),
           (val_mul,":loanline_addition",5),
           (val_div,":loanline_addition",100),
           (troop_set_slot,"trp_player",slot_troop_line_of_credit_addition,":loanline_addition"),			
    ]),
  ...
  (24,
   [
        (try_begin),
	   (gt,"$player_honor",0),
           (store_mul,":loanline",10000,"$player_honor"),
           (troop_get_slot,":loanline_addition","trp_player",slot_troop_line_of_credit_addition),
           (val_add,":loanline",":loanline_addition"),
           (troop_set_slot,"trp_player",slot_troop_line_of_credit,":loanline"),
	(else_try),
           (troop_get_slot,":loanline_addition","trp_player",slot_troop_line_of_credit_addition),
           (store_add,":loanline",5000,":loanline_addition"),
           (troop_set_slot,"trp_player",slot_troop_line_of_credit,":loanline"),
	(try_end),

	(troop_get_slot,":debt","trp_player",slot_troop_debt),
	(troop_get_slot,":loanline","trp_player",slot_troop_line_of_credit),
	(troop_get_slot,":debt_date","trp_player",slot_troop_debt_date),
	(try_begin),
	    (le,":loanline",":debt"),
		(try_begin),		
			(eq,":debt_date",-1),
			(store_current_day, ":debt_date"),
			(troop_set_slot,"trp_player",slot_troop_debt_date,":debt_date"),
		(try_end),
		(troop_get_slot,":debt_date","trp_player",slot_troop_debt_date),
		(try_begin),
			(neq,":debt_date",-1),
            (store_current_day, ":cur_day"),
			(store_sub,":time",":cur_day",":debt_date"),
			(eq,":time",10),
			(dialog_box,"@Notice! The Bank ask you to pay back the money! Otherwise They will take any measures"),
		(try_end),
		(try_begin),
			(neq,":debt_date",-1),
            (store_current_day, ":cur_day"),
			...
			(try_begin),
				(gt,":deposit",0),
				(troop_get_slot,":debt","trp_player",slot_troop_debt),
				(val_sub,":debt",":deposit"),
				(troop_set_slot,"trp_player",slot_troop_deposit,0),
				(troop_set_slot,"trp_player",slot_troop_debt,":debt"),
				(dialog_box,"@Notice! The Bank has confiscated your depositc in bank. If you don't pay back the money in time, they will take extreme measures!"),
			(try_end),
		(try_end),
		(try_begin),
			(neq,":debt_date",-1),
			...
			(ge,":time",20),
            (set_spawn_radius, 1),
            (spawn_around_party, "p_main_party", "pt_merchant_manhunters"),
            (party_get_position, pos1, "p_main_party"),
            (call_script, "script_map_get_random_position_around_position_within_range", 1, 2),
            (assign,":spawned_party",reg0),
            (party_set_position, ":spawned_party", pos2),
            (party_set_faction, ":spawned_party", "fac_merchants"),
            #(party_set_slot, ":spawned_party", slot_party_ai_object, "p_main_party"),
			#(party_set_slot, ":spawned_party", slot_party_ai_state, spai_screening_army),
			(party_set_ai_behavior, ":spawned_party", ai_bhvr_attack_party),
			(party_set_ai_object,":spawned_party","p_main_party"),
			(party_set_flags, ":spawned_party", pf_default_behavior, 0),
	        #(display_message,"@Avanger",0xFF4433),
            #(party_set_name, ":spawned_party", "@Avanger"), 
        (try_end),
	(else_try),
		(troop_set_slot,"trp_player",slot_troop_debt_date,-1),	
	(try_end),		
    ]),
 

## ZZ party change speed begin
   (1,
   [    
	(try_begin),
		(eq,"$current_party_state",1),
		(store_current_hours,":current_hour"),
		(store_sub,":dif",":current_hour","$march_hour"),
		(assign,reg2,":dif"),
		#(display_message,"@Now:{reg2}"),
		(store_div,":mod",reg2,24),
		(val_mul,":mod",":mod"),
		(val_mul,":mod",30),
		(val_div,":mod",-5),
		(call_script,"script_change_player_party_morale",":mod"),
	(else_try),
		...
		(try_begin),	
			...
			(display_message,"@Notice!Cover_up is now end.",0xFF0000),
		(try_end),				
    (try_end),		
	 ],
	),
## ZZ party change speed end
## ZZ change culture begin
(24,[
      (try_for_range, ":center_no", centers_begin, centers_end),
...
########  ZZ lord title ###############
## ZZ barrack changed by windmelodie for castle begin
  (72,
   [
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
		(party_set_slot,":center_no",slot_center_troop_1,1),
     (try_end),
    ]),
## ZZ barrack changed by windmelodie for castle end

## ZZ   patrol
 # (24,
  # [
	#(try_for_range, ":cur_party", villages_begin, villages_end), #towns_begin yifeng modify
	#	(party_get_slot,":patrol", ":cur_party",slot_party_patrol),
	#	(le,":patrol",0),
	...
	#	(party_clear, "p_temp_party"),
     ...
     #   (assign, ":tier_upgrades" , 5),
     #   (store_random_in_range, ":number_2", 2, 3),  #2,10
     #   (try_for_range, ":unused_2", 0, ":number_2"),  
	...
	#	  (try_end),
     #     (store_random_in_range, ":number_3", 5, 10),
     #     (party_add_members, "p_temp_party", ":volunteer_troop", ":number_3"),
	#	(try_end),
#
    #    (set_spawn_radius, 1),
    #    (spawn_around_party, ":cur_party", "pt_patrol_party"),
   #     (party_get_position, pos1, ":cur_party"),
    #    (call_script, "script_map_get_random_position_around_position_within_range", 1, 2),
    #    (assign,":spawned_party",reg0),
    #    (party_set_position, ":spawned_party", pos2),
  #
   #     (party_set_faction, ":spawned_party", ":cur_faction"),
   #     (party_set_slot, ":spawned_party", slot_party_type, spt_patrol),
   #     (party_set_slot, ":spawned_party", slot_party_ai_object, ":cur_party"),
   #     (party_set_slot, ":spawned_party", slot_party_ai_state, spai_patrolling_around_center),
	#	(party_set_ai_patrol_radius, ":spawned_party", 5),
	#	(party_set_aggressiveness,":spawned_party", 15),
	#	(party_set_courage, ":spawned_party", 15),	
  #      (str_store_party_name, s5, ":cur_party"),
   #     (party_set_name, ":spawned_party", "@{s5} patrol"),
  #      (call_script, "script_party_add_party", ":spawned_party", "p_temp_party"),
	#	(party_set_slot, ":cur_party",slot_party_patrol, ":spawned_party"),	
   # (try_end),		
  # 
 #  ]),


## Ellis desert  			(neq,":terrain",7),(neq,":terrain",15),
  (24,
   [
	(party_get_current_terrain, ":terrain_type", "p_main_party"),
	(try_begin),
	  (eq, ":terrain_type", rt_desert),
        (display_message, "@You spent too much time in desert!", 0xFF0000),
        (call_script, "script_change_player_party_morale", -5),
 
 (else_try),
	  (eq, ":terrain_type", 15),
              (display_message, "@You spent too much time in ocean!", 0xFF0000),
              (call_script, "script_change_player_party_morale", -5),
        (try_end),
...
####zhunbeishawang
   ...
   [
      (try_begin),
        (eq, "$zhunbeishawang", 1),
                (assign, "$zhunbeishawang", 0),
                (jump_to_menu, "mnu_bushawang"),
      (try_end),
...
###zhunbeishajiang
   ...
   [
      (try_begin),
        (eq, "$zhunbeishajiang", 1),
                (assign, "$zhunbeishajiang", 0),
        (try_for_range, ":chujuejiang", lords_begin, lords_end),#ceren
                (troop_slot_eq, ":chujuejiang", slot_troop_renown, 90000),
                (store_troop_faction, ":chujuejiang_faction", ":chujuejiang"),
                (faction_get_slot, ":duifangtoutou", ":chujuejiang_faction", slot_faction_leader),
                         (try_for_range, ":cur_center", centers_begin, centers_end),
       (store_faction_of_party, ":cur_faction", ":cur_center"),
       (eq, ":cur_faction", ":chujuejiang_faction"),
       (party_slot_eq, ":cur_center", slot_town_lord, ":chujuejiang"),
          # (assign, ":youlingdi", 1),
           (call_script, "script_give_center_to_lord", ":cur_center", ":duifangtoutou", 0),#chujuefulu
            (try_end),
        (call_script, "script_change_troop_faction", ":chujuejiang", "fac_outlaws"),
        (troop_set_faction, ":chujuejiang", "fac_outlaws"),
                (troop_get_slot, ":led_party", ":chujuejiang", slot_troop_leaded_party),
        (try_begin),
                (party_is_active, ":led_party"),
                (neq, ":led_party", "p_main_party"),
                (remove_party, ":led_party"),
                (troop_set_slot, ":chujuejiang", slot_troop_leaded_party, -1),
        (try_end),
        
        
        (troop_get_slot, ":renown1", "trp_player", slot_troop_renown),
          
          
           (store_sub, ":renown2", ":renown1", 35),
           (troop_set_slot, "trp_player", slot_troop_renown, ":renown2"),
        
    (try_end),#ceren
          (jump_to_menu, "mnu_chujuele"),
      (try_end),
    ]),
##end
      ...
      (try_begin),
         ...
         (try_for_range, ":village", villages_begin, villages_end),
            ...
            #Windmelodie change begin
                  ...
                  (eq, ":village_current_faction", ":recruit_faction"),
            #Windemelodie change end
                  (assign,":stop",0),
               (try_end),
               (neq,":stop",1),
      ...
      #daedalus begin
        ...
        #Windmelodie change begin
           #(eq, ":target_faction", ":recruit_faction"),  
            (eq, ":target_current_faction", ":recruit_faction"),
        #Windmelodie change end
            (assign,":stop",0),
            (try_end),
            (neq,":stop",1),
...
###Xp from old xx
 ...
 #Windmelodie begin
  # Offering ransom fees for player's prisoner heroes (asked)
  (24,
   [(eq, "$g_ransom_offer_troop_processing", 1),
    (assign, ":result", 0),
    (party_get_num_prisoner_stacks, ":num_stacks", "$g_ransom_offer_party_asked"), 
	(try_for_range, ":i_stack", 0, ":num_stacks"),
	    (eq, ":result", 0),
	    (party_prisoner_stack_get_troop_id, ":stack_troop", "$g_ransom_offer_party_asked", ":i_stack"),
		(eq, ":stack_troop", "$g_ransom_offer_troop_asked"),
		(assign, ":result", 1),
		(assign, ":num_stacks", 0),
	(try_end),	
    (try_begin),
        (eq, ":result", 1),
        (jump_to_menu, "mnu_enemy_offer_ransom_for_prisoner_asked"),
	(else_try),
	    (jump_to_menu, "mnu_enemy_offer_ransom_for_prisoner_asked_2"),
	(try_end),
	]), 
#Windmelodie end

## Windmelodie Call_npc begin
  (48,
	[(try_begin),
		(neg|faction_slot_eq,"fac_mod_variables",slot_Mod_NPC_Calling_NPC,0),
		(call_script, "script_cf_update_NPC",),
	(try_end),
	(faction_set_slot,"fac_mod_variables",slot_Mod_NPC_Calling_NPC,0),
  ]), 
## Windmelodie Call_npc end


## Windmelodie Patrol begin
  (24,
	[(try_begin),
   		(faction_slot_eq,"fac_mod_variables",slot_Mod_fort_patrol_reinforce,1),
		(faction_set_slot,"fac_mod_variables",slot_Mod_fort_patrol_reinforce,0),
	(else_try),
	]),
## Windmelodie Patrol end

## Windmelodie fort begin
  (24 * 7,
	[(store_random_in_range,":random",-2,2),
	(party_get_slot,":prosperity","p_fort", slot_town_prosperity),
	(val_add,":prosperity",":random"),
   	(party_set_slot, "p_fort", slot_town_prosperity,":prosperity"),
	]),
## Windmelodie fort end

## Windmelodie trade renew farmer begin
  (24 * 3,
	[
	(call_script, "script_trade_renew_farmer"),
	(call_script, "script_trade_renew_merchant"),
	]),
## Windmelodie trade renew farmer end

## Windmelodie Heroes banner begin
  (24 * 3,
	[
	(call_script, "script_renew_banner"),
	]),

## Windmelodie Heroes banner endswith
