...
game_menus = [
  ("start_game_0",menu_text_color(0xFF000000)|mnf_disable_all_keys,
    "Welcome,This mod is made by Century War team(main members are Mskimizd,Ellis and Yifeng).But we do python more than textures.because many models and textures from the OSP packs or other mods.If I used some copyrighted materials.I request your permission and pardon genuinely.We only want to let more people know the M&B and the history,nothing else.Thanks to all the authors who made materials.Finally,because of my bad English,the translation is not perfect.Thanks for download.",
    "none",
    [
	##yifeng npc
      (try_begin),
         (eq, "$g_get_more_money", 1),
...
##yifeng npc	
       ("option_1", 
         ...
         "{s1} Get_more_money.",
         [
           (val_add, "$g_get_more_money", 1),
           (val_mod, "$g_get_more_money", 2),
...
##yifeng npc		
  ...
  ("start_phase_2",mnf_disable_all_keys,
    "It is 16th century now, an age with fire and sword,\
 ...
 and that whatever course you take, great adventures will await you. Drawn by the stories you hear about this world and its kingdoms, you...",
    "none",
    [],
    [
      ("town_1",[(eq, "$current_startup_quest_phase", 0),],"join a caravan to London.",
       [
	 ...
	 (party_get_position, pos1, "p_town_1"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		 
#windmelodie troop begin #London
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_england_lancer_guard",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_england_veteran_horse_archer",3),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_england_tribesman",5),
		  (try_end),
#windmelodie troop end
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
#         (change_screen_return),
       ]),
       
      ("town_2",[(eq, "$current_startup_quest_phase", 0),],"join a caravan to Moscow.",
       [
         (assign, "$current_town", "p_town_10"),
         (assign, "$g_starting_town", "$current_town"),
         (assign, "$g_journey_string", "str_journey_to_reyvadin"),
	  ## ZZ Moscow
	 (party_get_position, pos1, "p_town_10"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		 
#windmelodie troop begin #Moscow
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_vaegir_knight",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_vaegir_archer",2),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_vaegir_recruit",5),
		  (try_end),
#windmelodie troop end
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
#         (change_screen_return),
      ...
      ("town_3",[(eq, "$current_startup_quest_phase", 0),],"take a ship to Vinice.",
       [
         (assign, "$current_town", "p_town_12"),
         (assign, "$g_starting_town", "$current_town"),
         (assign, "$g_journey_string", "str_journey_to_tulga"),
	  ## ZZ Rome
	 (party_get_position, pos1, "p_town_12"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		 
#windmelodie troop begin #Rome
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_nord_veteran",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_hench_halberdman",3),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_nord_recruit",5),
		  (try_end),
#windmelodie troop end
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
#         (change_screen_return),
      ...
      ("town_4",[(eq, "$current_startup_quest_phase", 0),],"take a ship to Jerusalem.",
       [
         (assign, "$current_town", "p_town_21"),
         (assign, "$g_starting_town", "$current_town"),
         (assign, "$g_journey_string", "str_journey_to_sargoth"),
	  ## ZZ Jerusalem
	 (party_get_position, pos1, "p_town_21"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		
#windmelodie troop begin #Jerusalem
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_sarranid_mamluke1",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_mamalik_guard",3),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_mamalik_recruit",5),
		  (try_end),
#windmelodie troop end 
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
#         (change_screen_return),
      ...
      ("town_5",[(eq, "$current_startup_quest_phase", 0),],"join a caravan to XiAn, in the land of China .",
       [
         (assign, "$current_town", "p_town_16"),
         (assign, "$g_starting_town", "$current_town"),
         (assign, "$g_journey_string", "str_journey_to_jelkala"),
	  ## ZZ China
	 (party_get_position, pos1, "p_town_16"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		 
#windmelodie troop begin #China
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_ming_veteran_horse_archer",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_divine_engine_division_ming_musketeers",3),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_ming_skirmisher",5),
		  (try_end),
#windmelodie troop end
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
#         (change_screen_return),
      ...
      ("town_6",[(eq, "$current_startup_quest_phase", 0),],"take a ship to Koyto.",
       [
         (assign, "$current_town", "p_town_23"),
         (assign, "$g_starting_town", "$current_town"),
         (assign, "$g_journey_string", "str_journey_to_shariz"),
	  ## ZZ kyoto
	 (party_get_position, pos1, "p_town_23"),
 	 (party_set_position, "p_main_party", pos1),
  	 (change_screen_map),
	  ## ZZ		 
#windmelodie troop begin #Kyoto
          (try_begin),
            (eq, "$background_type", cb_noble),
			(party_add_members, "p_main_party", "trp_japan_lancer_guard",2),
		  (else_try),
			(eq, "$background_type", cb_guard),
		    (party_add_members, "p_main_party", "trp_japan_crossbowman1",3),
		  (else_try),
		    (party_add_members, "p_main_party", "trp_japan_tribesman",5),
		  (try_end),
#windmelodie troop end
#	  (jump_to_menu, "mnu_start_phase_2_5"),
#         (party_relocate_near_party, "p_main_party", "$g_starting_town", 2),
...
#yifeng
           ("view_hungry_report",[],"Rapport de sant??.",
...
#hunger mod reports
    (try_begin),
        (eq, "$malus_de_faim", 0),
        (lt, "$malus_de_faim", 1),    
        (str_store_string, s3, "@Vous etes rassasi?? !"),
      (else_try),        
        (str_store_string, s3, "@Vous etes affam?? et vous vous sentez faible ! malus for -2/ ag -1"),#
    (try_end),
    (str_store_string, s1, "@Rapport de sant??:^{s3}"),
...
#wounds/death mod report azqs
    (try_begin),
      ...
      (else_try),   
        (str_store_string, s9, "@Vous etes bless?? a la jambe ! malus for -1/ ag -1"),    
    (try_end),
...
#
    ...
    (try_begin),
      ...
      (else_try),
        (str_store_string, s7, "@Vous etes bless?? au bras ! malus for -3/ ag -4"),    
    (try_end),
...
#
    ...
    (str_store_string, s5, "@intacte"),
      (else_try),
        (str_store_string, s5, "@Vous etes gri??vement bless?? a la t?ate ! malus for -1/ int -4/ cha -5/ equit/ -1"),    
    (try_end),
    (str_store_string, s4, "@T?ate: {s5}"),  
...
#Windmelodie Outpost Changed begin changed at 1.5.1
("build_outpost", 
			[(ge,"$player_right_to_rule",6),
			(troop_slot_ge, "trp_player", slot_troop_renown, 200),
			(party_slot_eq, "p_village_193", slot_outpost_act, 0),
			(try_begin),
				(party_get_current_terrain,":terrain","trp_player"),
				(this_or_next|eq,":terrain",rt_mountain),
				(this_or_next|eq,":terrain",rt_deep_ocean),
				(this_or_next|eq,":terrain",rt_water),
				(eq,":terrain",rt_bridge),
				(disable_menu_option),
			(try_end),
			], 
		    "Build an Outpost. (5000 denars)",
		    [(store_troop_gold, ":gold_amount", "trp_player"),
			 ...
			 (else_try),
				...
				(lt, ":cur_day", "$mercenary_service_next_renew_day"),
				(display_message, "str_cant_build_castle_during_mercenary_service"),
			(else_try),
		        (ge, ":gold_amount", 5000),
				(troop_remove_gold, "trp_player", 5000),
			    (enable_party, "p_village_193"),
			    (party_relocate_near_party, "p_village_193", "p_main_party"),
				(call_script, "script_give_center_to_faction_aux", "p_village_193", "fac_player_supporters_faction"),
				(call_script, "script_give_center_to_lord", "p_village_193", "trp_player", 0),
				(party_set_slot, "p_village_193", slot_center_accumulated_rents, 0),
				(party_set_slot, "p_village_193", slot_center_accumulated_tariffs, 0),
				(party_set_slot, "p_village_193", slot_outpost_act, 1),
				#Windmelodie add
				
				#(faction_set_slot,"fac_player_supporters_faction",slot_faction_state, sfs_active),
				#(party_set_faction, ":outpost_party", "fac_player_supporters_faction"),
				#(troop_get_slot, ":cur_banner", "trp_player", slot_troop_banner_scene_prop),
				#(try_begin),
				#	(gt, ":cur_banner", 0),
				#	(val_sub, ":cur_banner", banner_scene_props_begin),
				#	(val_add, ":cur_banner", banner_map_icons_begin),
				#	(party_set_banner_icon,":outpost_party", ":cur_banner"),
				#(try_end),
				#(party_set_slot, ":outpost_party", slot_town_lord, "trp_player"),
				#(party_set_slot, ":outpost_party", slot_party_type, spt_castle),
				#(party_set_slot, ":outpost_party", slot_outpost_level, 1),
				#(rest_for_hours, 6, 5, 1), # Lumos: A crude way to add waiting.
				(party_set_slot, "p_village_193", slot_outpost_patrol,0 ),
...
#Windmelodie Outpost Changed end
	   
	   
	   
	   
##windmelodie move outpost fort begin
      ("change_outpost_position",
		[(party_slot_eq, "p_village_193", slot_town_lord, "trp_player"),
		(str_store_party_name, s1, "p_village_193"),
		(try_begin),
			(store_troop_gold, ":gold_amount", "trp_player"),
			(party_get_current_terrain,":terrain","trp_player"),
			(this_or_next|eq,":terrain",rt_mountain),
			...
			(this_or_next|lt, ":gold_amount", 8000),
			(this_or_next|party_slot_eq, "p_village_193", slot_village_state, svs_being_raided),
			(party_slot_ge, "p_village_193", slot_village_infested_by_bandits, 1),
			(disable_menu_option),
		(try_end),
		],
		"Change {s1} position.(8000)",
		[(party_relocate_near_party, "p_village_193", "p_main_party"),
      ...
      ("change_fort_position",
		[(party_slot_eq, "p_castle_98", slot_town_lord, "trp_player"),
		(str_store_party_name, s1, "p_castle_98"),
		(try_begin),
			(store_troop_gold, ":gold_amount", "trp_player"),
			(party_get_current_terrain,":terrain","trp_player"),
			(this_or_next|eq,":terrain",rt_mountain),
			...
			(this_or_next|eq,":terrain",rt_water),
			(this_or_next|eq,":terrain",rt_bridge),
			(this_or_next|lt, ":gold_amount", 20000),
			(party_slot_ge, "p_castle_98", slot_center_is_besieged_by, 0),
			(disable_menu_option),
		(try_end),
		],
		"Change {s1} position.(20000)",
		[(party_relocate_near_party, "p_castle_98", "p_main_party"),
		(troop_remove_gold,"trp_player",20000),
		(change_screen_map), 
        ]
       ),	  
##windmelodie move outpost fort end
	   
	  ## ZZ Party Order 
      ("party_order",[],"Give order to party.",
       [
	   (jump_to_menu, "mnu_party_order"),
        ]
       ),	
      #("xxx",[],"xxx",
      # [		
	  #	   (assign,"$temp_troop","trp_swadian_knight"),   
      ...
      ("camp_cheat",
       ...
       #zs:money_to_morale
        ("money_to_morale",
        [
#            # Hide if morale is already full.
...
#            (gt, ":num_stacks", 1),
            ...
            # Disable if morale is already full.
            (party_get_morale, reg1, "p_main_party"),
            # Disable if only player.
            (party_get_num_companion_stacks, ":num_stacks", "p_main_party"),
            (try_begin),
                (this_or_next|ge, reg1, 99),
                (eq, ":num_stacks", 1),
                (disable_menu_option),
            (try_end),
        ],
        "Reward soldiers (morale: {reg1}).",
        [
            (jump_to_menu, "mnu_money_to_morale"),
        ]),
  ...
  ("camp_cheat",0,
   ...
   [
    ...
    [
## windmelodie mod options begin
      ("cheat_mod_options",[],"mod cheat options",
      [(start_presentation, "prsnt_cheat_mod_options"),]),
## windmelodie mod options end
   ...
   [
    ...
    [
      ...
      ("cheat_decrease_fog",[], "{!}Decrease Fog Amount.",
       ...
       ##HolyAce ÕĞÄ¼½¨Öİ¶¼¶½»¤¾ü####
 ("duduhujun",[],"{!}Zhaomu TeshuBingzhong.",
       [(jump_to_menu, "mnu_which_country"),
        ]
       ),
################################
...
##ZZ rebellion begin
      ("rebellion",
        [
			...
			#add by windmelodie 1.5.2
			(neg|party_slot_eq, "p_village_193", slot_outpost_act, 1),
			(neg|party_slot_eq, "p_village_193", slot_center_current_improvement, slot_center_has_castle),
			#add by windmelodie 1.5.2 end
			#added by windmelodie 1.56 20150206 begin
			(neg|faction_slot_eq, "fac_player_supporters_faction",slot_faction_leader, "trp_player"),
			#added by windmelodie 1.56 end
       ...
       [
        ...
        (store_troop_faction,":old_faction",":faction_leader"),
        (call_script, "script_change_player_relation_with_troop",":faction_leader", -50),
        (call_script, "script_change_player_relation_with_faction_ex", ":old_faction", -40),
        (try_for_range, ":cur_troop", lords_begin, lords_end),
            (store_troop_faction,":cur_faction",":cur_troop"),
            (try_begin),
		(eq,":cur_faction",":old_faction"),
		(troop_get_slot, ":reputation_type", ":cur_troop", slot_lord_reputation_type),
		(try_begin),
		    (this_or_next|eq,":reputation_type",lrep_cunning),
		    (eq,":reputation_type",lrep_debauched),
		    (call_script, "script_change_player_relation_with_troop", ":cur_troop", +10),
		(else_try),
		    (this_or_next|eq,":reputation_type",lrep_goodnatured),
		    (eq,":reputation_type",lrep_upstanding ),
		    (call_script, "script_change_player_relation_with_troop", ":cur_troop", -10),
		(else_try),
		    (call_script, "script_change_player_relation_with_troop", ":cur_troop", -2),
		(try_end),					
            (try_end),
        (try_end),
        (troop_get_slot, ":renown", "trp_player", slot_troop_renown),
        (val_sub,":renown",1000),
        (troop_set_slot, "trp_player", slot_troop_renown, ":renown"),
        (val_sub,"$player_honor",50),

        (call_script, "script_player_leave_faction", 0),
        (call_script, "script_activate_player_faction", "trp_player"),
        (call_script, "script_diplomacy_start_war_between_kingdoms", "$players_kingdom", ":old_faction", 0),
        (change_screen_return),
        ],
       ),
##ZZ rebellion end
...
## yifeng rename party
   ("action_change_party_name",
       [(troop_slot_ge, "trp_player", slot_troop_renown, 200)], #My opinion is that it should be renown-dependent...
...
#yifeng		   
      ...
      ("action_rename_kingdom",
       [
		 (assign,":result",0),
		 (try_begin),
		    (party_slot_ge, "p_outpost_1", slot_outpost_level, 1),
			(assign,":result",1),
		 (else_try),
  ...
  ("cattle_herd",mnf_scale_picture,
   ...
   [(play_sound, "snd_cow_moo"),
    ...
    [
      ...
      ("cattle_follow",[(faction_slot_eq,"fac_mod_variables",slot_Mod_cattle_follow	,1)],
	  "let the cattle follow you.",
       [
        (party_set_slot, "$g_encountered_party", slot_cattle_driven_by_player, 1),
...
############################################################################################################
	  ...
	  (party_get_current_terrain,":terrain","p_main_party"),
          (try_begin),		  
		(neq,":terrain",7),(neq,":terrain",15),		
		(set_jump_mission,"mt_lead_charge"),
		(call_script, "script_setup_random_scene"),
	  (else_try),	
		(this_or_next|eq,":terrain",7),(eq,":terrain",15),
...
###Troop commentary changes end
  ...
  (
    ...
    "{s11}^^Your Casualties:{s8}{s10}^^Enemy Casualties:{s9}^^Kill Stat:{s20}",#zs:kill_stat: local: menu_battle_debrief|
    "none",
    [
     (try_begin),
       (eq, "$g_battle_result", 1),
...
#zs:kill_stat add by windmelodie
    (str_clear, s20),
    (party_get_num_companion_stacks, ":num_stacks", "p_main_party"),
    (try_for_range, ":i_stack", 0, ":num_stacks"),
        (party_stack_get_troop_id, ":stack_troop", "p_main_party", ":i_stack"),
        (str_store_troop_name, s1, ":stack_troop"),
        (troop_get_slot, reg1, ":stack_troop", slot_troop_kill_cnt),#":troop_kill_cnt"
        (str_store_string, s20, "@{s20}^{s1}'s kill stat: {reg1}"),# local: qstr_{s20}^{s1}_s_kill_st|
    (try_end),
...
############################################################################################################
       #(troop_get_type, ":is_female", "trp_player"),
#yifeng       (troop_get_type, ":is_female", "$g_player_troop"),
############################################################################################################
##### Custom Commander(CC)
############################################################################################################
...
############################################################################################################
  ...
  (
    ...
    [
## windmelodie patrol begin
        ("patrol_management",
        [(party_slot_eq, "$g_encountered_party", slot_center_has_patrol_center, 1)],
        "manage your patrol",
        [(jump_to_menu,"mnu_patrol")]),	
## windmelodie patrol end
## ZZ Aid to develop begin
        ("aid_to_develop",
        [
            (neg|party_slot_eq, "$current_town", slot_village_state, svs_looted),
        ...
        "Aid the center to develop",
        [
            (try_begin),
                (call_script, "script_cf_enter_center_location_bandit_check"),
            (else_try),
                (jump_to_menu,"mnu_aid_to_develop"),
            (try_end),
        ]),	
## ZZ Aid to develop end
...
## ZZ change_village_troop_type_according_to_player_faction
        (call_script, "script_change_village_troop_type_according_to_player_faction","$g_encountered_party"),
## ZZ change_village_troop_type_according_to_player_faction
       ]
       ,"Recruit Volunteers.",
       [
...
############################################################################################################
      ...
      ("village_reports",[(eq, "$cheat_mode", 1),], "{!}CHEAT! Show reports.",
       [(jump_to_menu,"mnu_center_reports"),
        ...
        #zs:village_station_troops
        ("village_station_troops",
        [
            (neg|party_slot_eq, "$current_town", slot_village_state, svs_looted),
            (neg|party_slot_eq, "$current_town", slot_village_state, svs_being_raided),
            (neg|party_slot_ge, "$current_town", slot_village_infested_by_bandits, 1),
            (party_slot_ge, "$current_town", slot_center_player_relation, 50),
            #(party_slot_eq,"$current_town",slot_town_lord, "trp_player"),
        ],
 ...
 currently building {s7}. The building will be completed after {reg8} day{reg9?s:}.:}",
    ...
    [(assign, ":num_improvements", 0),
     ...
     (try_begin),
       ...
       (assign, ":begin2", village_new_improvements_begin),#add by windmelodie
       (assign, ":end2", village_new_improvements_end),#add by windmelodie
       (str_store_string, s17, "@village"),
     (else_try),
       (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
	   (assign, ":begin", walled_center_improvements_begin),
       (assign, ":end", walled_center_improvements_end),
	   (assign, ":begin2", town_new_improvements_begin),#add by windmelodie
       (assign, ":end2", town_new_improvements_end),#add by windmelodie
       (str_store_string, s17, "@town"),
     (else_try),
	   (assign, ":begin", walled_center_improvements_begin),
       (assign, ":end", walled_center_improvements_end),
	   (assign, ":begin2", castle_new_improvements_begin),#add by windmelodie
       (assign, ":end2", castle_new_improvements_end),#add by windmelodie
	 ...
	 #add by windmelodie begin
     (try_for_range, ":improvement_no", ":begin2", ":end2"),
       (party_slot_ge, "$g_encountered_party", ":improvement_no", 1),
       (val_add,  ":num_improvements", 1),
       (call_script, "script_get_improvement_details", ":improvement_no"),
       (try_begin),
	...
	#add by windmelodie begin
	  ("center_build_patrol_center",[(eq, reg6, 0),
                                 (eq, "$g_encountered_party","p_village_193"),
                                 (party_slot_eq, "$g_encountered_party", slot_center_has_patrol_center, 0),
                                  ],
       "Build a patrol center.",[
							(try_begin),
								(ge,"$player_right_to_rule",10),
								(assign, "$g_improvement_type", slot_center_has_patrol_center),
								(jump_to_menu, "mnu_center_improve"),
							(else_try),
								(display_message, "str_not_enough_rtr"),
							(try_end),
						]),
						
      ("center_build_castle",[(eq, reg6, 0),
                                 (eq, "$g_encountered_party","p_village_193"),
                                 (party_slot_eq, "$g_encountered_party", slot_center_has_castle, 0),
                                  ],
       "Build a castle.",[
	   #changed by windmelodie at 1.5.1 begin
							(try_begin),
...
#								(display_message, "str_cant_build_castle_during_mercenary_service"),
							(else_try),
								(assign, "$g_improvement_type", slot_center_has_castle),
								(jump_to_menu, "mnu_center_improve"),
							(try_end),
		#changed by windmelodie at 1.5.1 end
						]),
						
	  ("center_build_supply_center",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_supply_center, 0),
                                       ],
       "Build a supply center.",[(assign, "$g_improvement_type", slot_center_has_supply_center),
                                  (jump_to_menu, "mnu_center_improve"),]),		
								  
	  ("center_build_training_camp",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_training_camp, 0),
                                       ],
       "Build a training camp.",[(assign, "$g_improvement_type", slot_center_has_training_camp),
                                  (jump_to_menu, "mnu_center_improve"),]),	
 ...
 {reg5} denars and will take {reg6} days.",
    ...
    [(call_script, "script_get_improvement_details", "$g_improvement_type"),
     ...
     (store_div, ":improvement_time", ":improvement_cost", 1000),#yifeng
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
             ...
             (try_end),
#whore
 ## windmelodie whore begin
			(try_begin),
				(store_current_hours,":hours"),
				(faction_get_slot,":last","fac_mod_variables",slot_Mod_whore_hours),
				(val_add,":last",3),
				(ge,":hours",":last"),
				(call_script, "script_get_town_group"),
				(try_begin),
					(eq,reg0,0),
					(try_begin),
						(store_random_in_range, ":random", 0, 100),
						(lt,":random",20),
						(set_visitor, ":cur_entry", "trp_ribaude_1"),
						(val_add, ":cur_entry", 1),
					(try_end), 
					(try_begin),
						(store_random_in_range, ":random", 0, 100),
						(lt,":random",20),
						(set_visitor, ":cur_entry", "trp_ribaude_2"),
						(val_add, ":cur_entry", 1),
					(try_end), 
					(try_begin),
						(store_random_in_range, ":random", 0, 100),
						(lt,":random",20),
						(set_visitor, ":cur_entry", "trp_ribaude_3"),
						(val_add, ":cur_entry", 1),
					(try_end), 
				(else_try),
					(eq,reg0,1),
				...
				(try_end),
			(try_end),  
## windmelodie whore end
 ##windmelodie trade  begin
            (try_for_range,":cur_farmer",trade_new_goods_farmer_begin,trade_new_goods_farmer_end),
				(troop_slot_eq,":cur_farmer",slot_troop_new_good_town, "$current_town"),
				(set_visitor, ":cur_entry", ":cur_farmer"),
				(val_add, ":cur_entry", 1),
			(try_end), 
            (try_for_range,":cur_merchant",trade_new_goods_merchant_begin,trade_new_goods_merchant_end),
				(troop_slot_eq,":cur_merchant",slot_troop_new_good_town, "$current_town"),
				(set_visitor, ":cur_entry", ":cur_merchant"),
				(val_add, ":cur_entry", 1),
			(try_end), 
 ##windmelodie trade  end
            (try_begin),
               (set_visitor, ":cur_entry", "trp_doctor_1"),
               (val_add, ":cur_entry", 1),

            (try_end),       
			
			(try_begin),
               (set_visitor, ":cur_entry", "trp_ransom_broker_10"),
               (val_add, ":cur_entry", 1),
...
################################## just copie and report this, at this place
#
  #           (set_visitor, 4, "trp_ribaude_1"),
 #            (set_visitor, 5, "trp_ribaude_2"),
 #            (set_visitor, 8, "trp_ribaude_3"),
 #            (assign, "$town_entered", 1),
 ##################################               
...
## ZZ downtown begin
 ...
 #     [(party_slot_eq, "$current_town", slot_party_type, spt_castle)],
  #    "Visit the Downtown",
   #   [
  #      (jump_to_menu, "mnu_downtown"),
#      ]),	  
## ZZ downtown end

## ZZ barrack begin
      ("barrack",
      [(party_get_slot,":num","$current_town",slot_center_troop_1),(ge,":num",0),],
      "Visit the barrack",
      [
        (jump_to_menu, "mnu_barrack"),
      ]),	  
## ZZ barrack end       

## ZZ India trade begin
        #("india_trade",
		#[
		#	(this_or_next|eq,"$current_town","p_castle_65"),
		#	(this_or_next|eq,"$current_town","p_castle_66"),
		#	(this_or_next|eq,"$current_town","p_castle_86"),#add  yifeng
		#	(this_or_next|eq,"$current_town","p_castle_87"),
		#	(this_or_next|eq,"$current_town","p_castle_88"),
		#	(this_or_next|eq,"$current_town","p_castle_89"),
		#	(this_or_next|eq,"$current_town","p_castle_90"),
		#	(this_or_next|eq,"$current_town","p_castle_113"),
		#	(this_or_next|eq,"$current_town","p_castle_119"),
		#	(this_or_next|eq,"$current_town","p_castle_132"),
		#	(this_or_next|eq,"$current_town","p_castle_135"),

		#	(eq,"$current_town","p_castle_67"),			
		#],
        #"Trade with India.",
        #[
        #    (jump_to_menu, "mnu_india_trade"),
        #]),
## ZZ India trade end
...
# They must learn field discipline and the steadiness to follow orders in combat before they can be thought to use arms.",
 ...
 #windmelodie castle trade
		[(party_slot_ge, "$current_town", slot_town_merchant, 1),
		(try_begin),
			(is_between, "$current_town",castles_begin,castles_end),
			(this_or_next|party_slot_eq, "$current_town", slot_center_has_market, 0),
			(neg|party_slot_ge, "$current_town", slot_center_player_relation, 6),
            (disable_menu_option),
		(try_end),
		],
 #windmelodie castle trade end
     ...
     ("downtown_bank", ## ZZ bank
      [	
		(party_slot_eq,"$current_town",slot_town_has_bank,1),
		],
		"Go to bank",
		[			 
...
## ZZ  develop begin changed by windmelodie
        ("aid_to_develop", [(is_between, "$current_town",walled_centers_begin, walled_centers_end)],
        "Aid the center to develop.",
        [
            (try_begin),
                (call_script, "script_cf_enter_center_location_bandit_check"),
...
## ZZ Aid to develop end changed by windmelodie
    ...
    [
      ...
      [      
        ...
        (try_begin),
          ...
          (gt, reg0, 0),          
          (troop_sort_inventory, "trp_temp_troop"),
## ZZ COMPANIONS OVERSEER by LAV
          #(assign, "$g_lco_auto_menu", "mnu_total_victory"),
          #(assign, "$g_lco_page", 2),
...
What kind of recruits do you want?",
   ...
   (str_store_faction_name,s7,"fac_kingdom_7"),
   (str_store_faction_name,s8,"fac_kingdom_8"),
   (str_store_faction_name,s9,"fac_kingdom_9"),
   (str_store_faction_name,s10,"fac_kingdom_10"),
   (str_store_faction_name,s11,"fac_kingdom_11"),
   (str_store_faction_name,s12,"fac_kingdom_12"),
   (str_store_faction_name,s13,"fac_kingdom_13"),
   (str_store_faction_name,s14,"fac_kingdom_14"),
   (str_store_faction_name,s15,"fac_kingdom_15"),
   (str_store_faction_name,s16,"fac_kingdom_16"),
   (str_store_faction_name,s17,"fac_kingdom_17"),
   (str_store_faction_name,s18,"fac_kingdom_18"),
   (str_store_faction_name,s19,"fac_kingdom_19"),
   (str_store_faction_name,s20,"fac_kingdom_20"),
   (str_store_faction_name,s21,"fac_kingdom_21"),
   (str_store_faction_name,s22,"fac_kingdom_22"),
   (str_store_faction_name,s23,"fac_kingdom_23"),
   (str_store_faction_name,s24,"fac_kingdom_24"),
   (str_store_faction_name,s25,"fac_kingdom_25"),
   (str_store_faction_name,s26,"fac_kingdom_26"),
   (str_store_faction_name,s27,"fac_kingdom_27"),
   (str_store_faction_name,s28,"fac_kingdom_28"),
   (str_store_faction_name,s29,"fac_kingdom_29"),
   (str_store_faction_name,s30,"fac_kingdom_30"),   
   ],
   ...
   ("recruiter_faction_0",  [(eq, "$recruiter_amount_check", 0)],
         "{s0}",               [(assign,"$temp","fac_player_supporters_faction"),(jump_to_menu, "mnu_recruiter_payment_2")]),
     
   ("recruiter_faction_1",  [(eq, "$recruiter_amount_check", 0)],
         "{s1}",               [(assign,"$temp","fac_kingdom_1"),(jump_to_menu, "mnu_recruiter_payment_2")]),

   ("recruiter_faction_2", [(eq, "$recruiter_amount_check", 0)],
         "{s2}",              [(assign,"$temp","fac_kingdom_2"),(jump_to_menu, "mnu_recruiter_payment_2")]),

   ("recruiter_faction_3", [(eq, "$recruiter_amount_check", 0)],
         "{s3}",              [(assign,"$temp","fac_kingdom_3"),(jump_to_menu, "mnu_recruiter_payment_2")]),

   ("recruiter_faction_4", [(eq, "$recruiter_amount_check", 0)],
         "{s4}",              [(assign,"$temp","fac_kingdom_4"),(jump_to_menu, "mnu_recruiter_payment_2")]),

   ("recruiter_faction_5", [(eq, "$recruiter_amount_check", 0)],
         "{s5}",              [(assign,"$temp","fac_kingdom_5"),(jump_to_menu, "mnu_recruiter_payment_2")]),
      
   ("recruiter_faction_6", [(eq, "$recruiter_amount_check", 0)],
         "{s6}",              [(assign,"$temp","fac_kingdom_6"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_7", [(eq, "$recruiter_amount_check", 0)],
         "{s7}",              [(assign,"$temp","fac_kingdom_7"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_8", [(eq, "$recruiter_amount_check", 0)],
         "{s8}",              [(assign,"$temp","fac_kingdom_8"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_9", [(eq, "$recruiter_amount_check", 0)],
         "{s9}",              [(assign,"$temp","fac_kingdom_9"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_next", [(eq, "$recruiter_amount_check", 0)],
         "next page",              [(jump_to_menu, "mnu_recruiter_payment1")])]),


   ("recruiter_payment1",0,
    "The recruiter will visit villages and recruit population to your forces. \
After he has collected the amount you ordered he returns to this {reg0?town:castle} and puts the recruits in the garrison. \
There's a limit for recruiters, which is 2 for castles and 4 for towns. \
What kind of recruits do you want?",
   ...
   [("recruiter_frount", [(eq, "$recruiter_amount_check", 0)],
         "frount page",              [(jump_to_menu, "mnu_recruiter_payment")]),
   ("recruiter_faction_10", [(eq, "$recruiter_amount_check", 0)],
         "{s10}",              [(assign,"$temp","fac_kingdom_10"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_11", [(eq, "$recruiter_amount_check", 0)],
         "{s11}",              [(assign,"$temp","fac_kingdom_11"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_12", [(eq, "$recruiter_amount_check", 0)],
         "{s12}",              [(assign,"$temp","fac_kingdom_12"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_13", [(eq, "$recruiter_amount_check", 0)],
         "{s13}",              [(assign,"$temp","fac_kingdom_13"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_14", [(eq, "$recruiter_amount_check", 0)],
         "{s14}",              [(assign,"$temp","fac_kingdom_14"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_15", [(eq, "$recruiter_amount_check", 0)],
         "{s15}",              [(assign,"$temp","fac_kingdom_15"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_16", [(eq, "$recruiter_amount_check", 0)],
         "{s16}",              [(assign,"$temp","fac_kingdom_16"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_17", [(eq, "$recruiter_amount_check", 0)],
         "{s17}",              [(assign,"$temp","fac_kingdom_17"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_18", [(eq, "$recruiter_amount_check", 0)],
         "{s18}",              [(assign,"$temp","fac_kingdom_18"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_19", [(eq, "$recruiter_amount_check", 0)],
         "{s19}",              [(assign,"$temp","fac_kingdom_19"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_next", [(eq, "$recruiter_amount_check", 0)],
         "next page",              [(jump_to_menu, "mnu_recruiter_payment2")])]),



   ("recruiter_payment2",0,
    "The recruiter will visit villages and recruit population to your forces. \
After he has collected the amount you ordered he returns to this {reg0?town:castle} and puts the recruits in the garrison. \
There's a limit for recruiters, which is 2 for castles and 4 for towns. \
What kind of recruits do you want?",
   ...
   [("recruiter_frount", [(eq, "$recruiter_amount_check", 0)],
         "frount page",              [(jump_to_menu, "mnu_recruiter_payment1")]),
   ("recruiter_faction_20", [(eq, "$recruiter_amount_check", 0)],
         "{s20}",              [(assign,"$temp","fac_kingdom_20"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_21", [(eq, "$recruiter_amount_check", 0)],
         "{s21}",              [(assign,"$temp","fac_kingdom_21"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_22", [(eq, "$recruiter_amount_check", 0)],
         "{s22}",              [(assign,"$temp","fac_kingdom_22"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_23", [(eq, "$recruiter_amount_check", 0)],
         "{s23}",              [(assign,"$temp","fac_kingdom_23"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_24", [(eq, "$recruiter_amount_check", 0)],
         "{s24}",              [(assign,"$temp","fac_kingdom_24"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_25", [(eq, "$recruiter_amount_check", 0)],
         "{s25}",              [(assign,"$temp","fac_kingdom_25"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_26", [(eq, "$recruiter_amount_check", 0)],
         "{s26}",              [(assign,"$temp","fac_kingdom_26"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_27", [(eq, "$recruiter_amount_check", 0)],
         "{s27}",              [(assign,"$temp","fac_kingdom_27"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_28", [(eq, "$recruiter_amount_check", 0)],
         "{s28}",              [(assign,"$temp","fac_kingdom_28"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_29", [(eq, "$recruiter_amount_check", 0)],
         "{s29}",              [(assign,"$temp","fac_kingdom_29"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_30", [(eq, "$recruiter_amount_check", 0)],
         "{s30}",              [(assign,"$temp","fac_kingdom_30"),(jump_to_menu, "mnu_recruiter_payment_2")]),
   ("recruiter_faction_31", [(eq, "$recruiter_amount_check", 0)],
         "Any troops",              [(assign,"$temp",-1), (jump_to_menu, "mnu_recruiter_payment_2")])]),




   ## Windmelodie change end 
...
##yifeng npc
 ...
 ## ZZ bank begin	
    ...
    [  
      ...
      ("save_box",[],"Save Box.",
       [
		(party_get_slot,":troop_manager","$current_town",slot_bank_manager),	   
	    (change_screen_loot, ":troop_manager"),
        ]
       ),
    ...
    [  		
	  ("village",[],"Village.",
       [	   
        ...
        (reset_visitors),	
		(set_visitor,11,"trp_village_88_elder"),		
		(set_visitor,9,"trp_khergit_castle_guard"),	
		#(set_visitor,36,"trp_town_walker_1"),	
		(set_visitor,34,"trp_khergit_castle_guard"),
		
	    (set_jump_mission,"mt_manor"),
	    (jump_to_scene, "scn_manor"),
        (change_screen_mission),		
        ]),	
		
	  ("manor_wait",[],"manor wait.",
		[
		 (rest_for_hours_interactive, 24, 5, 0),
         (change_screen_return),
		 ]),
...
## ZZ Party Order begin
    ("party_order",0,
    "You can give your party a order.",
    "none",
    [],
    [  		
        ("fast", [(eq,"$current_party_state",0),], "fast.",
        [(assign,"$current_party_state",1),
		 (call_script,"script_change_player_party_morale",-20),
		 (store_current_hours,"$march_hour"),
		 (display_message,"@Now your party now can move fast.But that will decrease your partie's morale.Notice!",0x0000EE),			 
		 ]),
		
        ("slow", [(eq,"$current_party_state",1),], "slow.",
        [(assign,"$current_party_state",0),]),

        ("cover_up", 
		[
		 (eq,"$current_party_state",0),
         (store_skill_level, ":leadership_level", "skl_leadership", "trp_player"),
		 (ge,":leadership_level",3),
		], "cover_up.",
        [(assign,"$current_party_state",2),
         (store_skill_level, ":leadership_level", "skl_leadership", "trp_player"),
		 (store_mul,"$time_temp",":leadership_level",3),
		 (assign,reg1,"$time_temp"),		 	 
		 (try_for_parties, ":party_no"),
			   (party_is_active, ":party_no"),		 
		       (party_ignore_player, ":party_no", "$time_temp"), 
		 (try_end),
		 (call_script,"script_change_player_party_morale",-20),
		 (store_current_hours,"$march_hour"),
		 (display_message,"@Now your party now can cover_up, but according to your leadship, it will only last {reg1} hours",0x0000EE),		 
		 ]),

        ("show", [(eq,"$current_party_state",2),], "show.",
        [(assign,"$current_party_state",0),]),		 
...
## ZZ Aid to develop begin
    ("aid_to_develop", 0,
    "{s10}'s prosperity: {reg10}, relation: {reg11}. ^You can give the center golds or prisoners.",
    "none",
    [
        ...
        (assign, reg12, 0),# town
        (assign, reg13, 1000),# town base gold
        (try_begin),
            (party_slot_eq, "$current_town", slot_party_type, spt_village),
            (assign, reg12, 1),# village
            (assign, reg13, 500),# village base gold
        (try_end),
    ],
    [
        ("gift_gold",
        ...
        [
            ...
            (try_for_range, ":unused", 0, 5),
                ...
                (display_message, "@Gift {reg1} {s1} as slaves."),
            (try_end),
...
########  ZZ lord title ###############  	   
      ("special_change_name",[],"Change Name",
       [
...
#minimap_info  disabled by windmelodie
	  ("open_minimap_info",[(neq, "$minimap_state",2),(eq, 1, 0)],"Open Minimap Info",
       [(assign, "$minimap_state",2),
        ]),
		
      ("close_minimap_info",[(neq, "$minimap_state",1),(eq, 1, 0)],"Close Minimap Info",
       [(assign, "$minimap_state",1),
        ]),	

#npc cmplaints	disabled by windmelodie	
      ("disable_npc_complaints",[(eq, "$disable_npc_complaints", 0),(eq, 1, 0)],"Close npc complaints.",
      [(assign, "$disable_npc_complaints", 1),]),

      ("enable_npc_complaints",[(eq, "$disable_npc_complaints", 1),(eq, 1, 0)],"Open npc complaints.",
      [(assign, "$disable_npc_complaints", 0),]),	


## windmelodie mod options begin
      ("more_mod_options",[],"more options",
      [(start_presentation, "prsnt_more_mod_options"),]),
## windmelodie mod options end

## windmelodie initialize relationship begin
      ("initialize_relationship",[(eq, "$cheat_mode",1),],"initialize relationship",
      [
	  (call_script,"script_set_player_relation_with_faction","fac_player_supporters_faction",90),
	  ]),
## windmelodie initialize relationship end
	  
 ## ZZ 16th war fog
      ("disable_war_fog",[(eq, "$disable_war_fog", 0),],"Close War Fog.",
      [(assign, "$disable_war_fog", 1),]),
      ## ZZ 16th war fog
...
## ZZ barrack changed by windmelodie begin
    ("barrack",0,
    "Here is the barrack of {s2}.",
    "none",
    [(str_store_party_name, s2, "$current_town"),],
    [  		
		("join_a",
			[(is_between,"$current_town",towns_begin,towns_end),],
           "Have a look at some new warriors",
			[(jump_to_menu, "mnu_reinforcements_a"),]
		),	
		
		("join_b",
			[(is_between,"$current_town",towns_begin,towns_end),],
           "Have a look at some warriors",
			[(jump_to_menu, "mnu_reinforcements_b"),]
		),	
	
		("join_c",
			[(is_between,"$current_town",castles_begin,castles_end),],
           "Have a look at some senior warriors",
			[(jump_to_menu, "mnu_reinforcements_c"),]
		),	
          
		("forget_it",[],"Forget it.",
			[
			 (try_begin),
				(ge,"$current_town","p_fort"),
...
## ZZ barrack changed by windmelodie end


## ZZ india_trade
    ("india_trade",0,
    "Trade with india with only goods",
    "none",
    [
       ...
       (try_for_range, ":i_slot", 0, ":end_cond"),
         ...
         (item_get_slot, ":price",":item_id", slot_item_price),
         (val_add, ":total_price", ":price"),
       (try_end),
	   (assign,reg10,":total_price"),
	],
	[
		("trade_rare_goods_1",[(ge,reg10,1100),],"Trade Rare Goods 1.",
		[
			(call_script,"script_trade_with_india",1100,"itm_rare_goods_1")
		]),
		
		("trade_rare_goods_2",[(ge,reg10,2000),],"Trade Rare Goods 2.",
		[
			(call_script,"script_trade_with_india",2000,"itm_rare_goods_2")
		]),

		("trade_rare_goods_3",[(ge,reg10,3000),],"Trade Rare Goods 3.",
		[
			(call_script,"script_trade_with_india",3000,"itm_rare_goods_3")
		]),

		("trade_rare_goods_4",[(ge,reg10,4000),],"Trade Rare Goods 4.",
		[
			(call_script,"script_trade_with_india",4000,"itm_rare_goods_4")
		]),

		("trade_rare_goods_5",[(ge,reg10,5000),],"Trade Rare Goods 5.",
		[
			(call_script,"script_trade_with_india",5000,"itm_rare_goods_5")
		]),	
...
[
    # Clear potential troops first. Don't use 0 to init, because it's trp_player.
    (assign, reg11, -1),
    (assign, reg12, -1),
    (assign, reg13, -1),
    (assign, reg14, -1),
    ...
    # $cur_page is 0 initially
    (store_mul, ":hero_pos1", "$cur_page", 9),# current page should begin with the $cur_page*9+1 hero.
    (val_add, ":hero_pos1", 1),
    (store_add, ":hero_pos2", ":hero_pos1", 1),
    (store_add, ":hero_pos3", ":hero_pos1", 2),
    (store_add, ":hero_pos4", ":hero_pos1", 3),
    (store_add, ":hero_pos5", ":hero_pos1", 4),
    (store_add, ":hero_pos6", ":hero_pos1", 5),
    (store_add, ":hero_pos7", ":hero_pos1", 6),
    (store_add, ":hero_pos8", ":hero_pos1", 7),
    (store_add, ":hero_pos9", ":hero_pos1", 8),
...
#    ("blacksmith_trp_player", [                                     (str_store_troop_name, s10, "trp_player")], "{s10}", [(assign, "$temp", "trp_player"), (jump_to_menu, "mnu_blacksmith2")]),
#    ("blacksmith_trp_npc1",   [(main_party_has_troop, "trp_npc1"),  (str_store_troop_name, s10, "trp_npc1")],   "{s10}", [(assign, "$temp", "trp_npc1"),   (jump_to_menu, "mnu_blacksmith2")]),
#...
#    ("blacksmith_trp_npc16",  [(main_party_has_troop, "trp_npc16"), (str_store_troop_name, s10, "trp_npc16")],  "{s10}", [(assign, "$temp", "trp_npc16"),  (jump_to_menu, "mnu_blacksmith2")]),
]),

#zs:blacksmith
("blacksmith2", 0, "What do you want to do?", "none",
[
    (store_item_kind_count, reg20, "itm_iron"),# reg20 = player iron count
    (store_item_kind_count, reg21, "itm_tools"),# reg21 = player tools count
    (store_troop_gold, reg22, "trp_player"),# reg22 = player gold
],
[
    # item_0
    ("blacksmith_ek_item_0",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_item_0),
        #reg10 = has item in the slot?
        #reg11 = item can be upgraded? 
        (try_begin),
            (this_or_next|eq, reg12, 0),#reg12 = troop can use the upgraded item?
            (this_or_next|lt, reg20, reg13),#reg13 = iron & tools count needed
            (this_or_next|lt, reg21, reg13),#reg13 = iron & tools count needed
            (lt, reg22, reg14),#reg14 = gold needed
            (disable_menu_option),
        (try_end),
        # Must backup for menu click event, otherwise reg13, reg14, reg15 will be overwriten by other ek menus.
        (assign, reg30, reg13),#reg13 = iron & tools count needed
        (assign, reg31, reg14),#reg14 = gold needed
        (assign, reg32, reg15),#reg15 = best imod
    ],
    "Weapon slot 1: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg30),
        (troop_remove_items, "trp_player", "itm_tools", reg30),
        (troop_remove_gold, "trp_player", reg31),
        (troop_set_inventory_slot_modifier, "$temp", ek_item_0, reg32),
    ]),

    # item_1
    ("blacksmith_ek_item_1",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_item_1),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
            (lt, reg22, reg14),
            (disable_menu_option),
        (try_end),
        (assign, reg33, reg13),
    ...
    "Weapon slot 2: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg33),
        (troop_remove_items, "trp_player", "itm_tools", reg33),
        (troop_remove_gold, "trp_player", reg34),
        (troop_set_inventory_slot_modifier, "$temp", ek_item_1, reg35),
    ]),

    # item_2
    ("blacksmith_ek_item_2",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_item_2),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "Weapon slot 3: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg36),
        (troop_remove_items, "trp_player", "itm_tools", reg36),
        (troop_remove_gold, "trp_player", reg37),
        (troop_set_inventory_slot_modifier, "$temp", ek_item_2, reg38),
    ]),

    # item_3
    ("blacksmith_ek_item_3",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_item_3),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "Weapon slot 4: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg39),
        (troop_remove_items, "trp_player", "itm_tools", reg39),
        (troop_remove_gold, "trp_player", reg40),
        (troop_set_inventory_slot_modifier, "$temp", ek_item_3, reg41),
    ]),

    # head
    ("blacksmith_ek_head",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_head),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "Head: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg42),
        (troop_remove_items, "trp_player", "itm_tools", reg42),
        (troop_remove_gold, "trp_player", reg43),
        (troop_set_inventory_slot_modifier, "$temp", ek_head, reg44),
    ]),

    # body
    ("blacksmith_ek_body",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_body),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "Body: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg45),
        (troop_remove_items, "trp_player", "itm_tools", reg45),
        (troop_remove_gold, "trp_player", reg46),
        (troop_set_inventory_slot_modifier, "$temp", ek_body, reg47),
    ]),

    # foot
    ("blacksmith_ek_foot",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_foot),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "foot: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg48),
        (troop_remove_items, "trp_player", "itm_tools", reg48),
        (troop_remove_gold, "trp_player", reg49),
        (troop_set_inventory_slot_modifier, "$temp", ek_foot, reg50),
    ]),

    # gloves
    ("blacksmith_ek_gloves",
    [
        (call_script, "script_equipment_upgrade_requirement", "$temp", ek_gloves),
        (try_begin),
            (this_or_next|eq, reg12, 0),
            (this_or_next|lt, reg20, reg13),
            (this_or_next|lt, reg21, reg13),
    ...
    "gloves: {reg10?{reg11?({s11} => {s12}){s10} ({reg13?{reg13} iron, {reg13} tools, :}{reg14} denars):{s11} {s10}}:None}",
    [
        (troop_remove_items, "trp_player", "itm_iron",  reg51),
        (troop_remove_items, "trp_player", "itm_tools", reg51),
        (troop_remove_gold, "trp_player", reg52),
        (troop_set_inventory_slot_modifier, "$temp", ek_gloves, reg53),
    ]),

#        # Body
#        ("blacksmith_ek_body",
#        [
#            (troop_get_inventory_slot, ":item_no", "$temp", ek_body),
#            (gt, ":item_no", 0),
#
#            (troop_get_inventory_slot_modifier, ":item_mod", "$temp", ek_body),
#            (neq, ":item_mod", imod_lordly),
#
#            (str_store_item_name, s10, ":item_no"),
#            (store_item_value, reg11, ":item_no"),
#            (val_mul, reg11, 5),
#
#            (try_begin), (lt, reg20, 6),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg21, 6),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg22, reg11), (disable_menu_option), (try_end),
#        ],
#        "Body: Upgrade to Lordly {s10} (6 iron, 6 tools, {reg11} denars)",
#        [
#            (troop_remove_items, "trp_player", "itm_iron",  6),
#            (troop_remove_items, "trp_player", "itm_tools", 6),
#            (troop_remove_gold, "trp_player", reg11),
#            (troop_set_inventory_slot_modifier, "$temp", ek_body, imod_lordly),
#        ]),
#        # Foot
#        ("blacksmith_ek_foot",
#        [
#            (troop_get_inventory_slot, ":item_no", "$temp", ek_foot),
#            (gt, ":item_no", 0),
#
#            (troop_get_inventory_slot_modifier, ":item_mod", "$temp", ek_foot),
#            (neq, ":item_mod", imod_lordly),
#
#            (str_store_item_name, s10, ":item_no"),
#            (store_item_value, reg12, ":item_no"),
#            (val_mul, reg12, 5),
#
#            (try_begin), (lt, reg20, 3),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg21, 3),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg22, reg12), (disable_menu_option), (try_end),
#        ],
#        "Foot: Upgrade to Lordly {s10} (3 iron, 3 tools, {reg12} denars)",
#        [
#            (troop_remove_items, "trp_player", "itm_iron",  3),
#            (troop_remove_items, "trp_player", "itm_tools", 3),
#            (troop_remove_gold, "trp_player", reg12),
#            (troop_set_inventory_slot_modifier, "$temp", ek_foot, imod_lordly),
#        ]),
#        # Gloves
#        ("blacksmith_ek_gloves",
#        [
#            (troop_get_inventory_slot, ":item_no", "$temp", ek_gloves),
#            (gt, ":item_no", 0),
#
#            (troop_get_inventory_slot_modifier, ":item_mod", "$temp", ek_gloves),
#            (neq, ":item_mod", imod_lordly),
#
#            (str_store_item_name, s10, ":item_no"),
#            (store_item_value, reg13, ":item_no"),
#            (val_mul, reg13, 5),
#
#            (try_begin), (lt, reg20, 2),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg21, 2),     (disable_menu_option), (try_end),
#            (try_begin), (lt, reg22, reg13), (disable_menu_option), (try_end),
#        ],
#        "Gloves: Upgrade to Lordly {s10} (2 iron, 2 tools, {reg13} denars)",
#        [
#            (troop_remove_items, "trp_player", "itm_iron",  2),
#            (troop_remove_items, "trp_player", "itm_tools", 2),
#            (troop_remove_gold, "trp_player", reg13),
#            (troop_set_inventory_slot_modifier, "$temp", ek_gloves, imod_lordly),
#        ]),
    ...
    #zs:money_to_morale
    ("money_to_morale", 0, "How to reward your soldiers?", "none",
        [
            # The original game_get_total_wage script is a shortcut for getting party weekly wage.
            # But it has been modified to get player faction wage in zs:correct_weekly_wage, so I've to copy&paste original code here.
            (assign, ":total_wage", 0),
            (party_get_num_companion_stacks, ":num_stacks", "p_main_party"),
            (try_for_range, ":i_stack", 0, ":num_stacks"),
                (party_stack_get_troop_id, ":stack_troop", "p_main_party", ":i_stack"),
            ...
            # Daily wage/2 as benchmark
            (val_div, ":total_wage", 7 * 2),
            (try_begin),
                (lt, ":total_wage", 1),
                (assign, ":total_wage", 1),# Set min benchmark to 1 to avoid 0 gold spending.
            (try_end),

            (store_mul, "$temp", ":total_wage", 1),
            (store_mul, "$temp_2", ":total_wage", 2),
            (store_mul, "$temp_3", ":total_wage", 3),
        ],
        [
            # Daily wage/2 to increase 0~20 morale
            ("money_to_morale_1",
                [
                    (store_troop_gold, ":player_gold", "trp_player"),
                    (ge, ":player_gold", "$temp"),
                ...
                "Closefisted ({reg1} denars).",
                [
                    (store_random_in_range, ":morale_increase", 0, 20 * 1),# Just random morale increased, no need to random spending, too.
                    (call_script, "script_change_player_party_morale", ":morale_increase"),
                    (troop_remove_gold, "trp_player", "$temp"),#(call_script, "script_troop_add_gold", "trp_player", ":remove_gold_1"),
                    (jump_to_menu, "mnu_camp"),
                ]
            ),
            # Daily wage to increase 20~40 morale
            ("money_to_morale_2",
                [
                    (store_troop_gold, ":player_gold", "trp_player"),
                    (ge, ":player_gold", "$temp_2"),
                    (assign, reg1, "$temp_2"),
                ],
                "Common ({reg1} denars).",
                [
            ...
            # Daily wage * 1.5 to increase 40~60 morale
            ("money_to_morale_3",
                [
                    (store_troop_gold, ":player_gold", "trp_player"),
                    (ge, ":player_gold", "$temp_3"),
                    (assign, reg1, "$temp_3"),
                ],
                "Generous ({reg1} denars).",
                [
                    (store_random_in_range, ":morale_increase", 20 * 2, 20 * 3),
                    (call_script, "script_change_player_party_morale", ":morale_increase"),
...
## windmelodie whore begin 
       ( "whore_1",mnf_scale_picture,
          "U have experienced a wonderfull night.",
          "none",
         [	(store_random_in_range, ":random", 0, 100),
			(try_begin),
				(lt,":random",33),
       ...
       ( "whore_3",mnf_scale_picture,
          "some knights want to join you",
          "none",
         [	(store_random_in_range, ":random", 0, 100),
			(try_begin),
				(lt,":random",33),
		 ...
		 ("whore_give_1000",
		 [],
		 "accept them (cost 1000).",             
         [
		 (store_troop_gold,":player_gold", "trp_player"),
		(try_begin),
			(ge, ":player_gold", 1000),
			(troop_remove_gold,"trp_player",1000),
			(call_script, "script_whore_knight",1),
			(store_random_in_range, reg1, 5,11),
			(party_add_members, "p_main_party", reg0, reg1),
			(display_message, "str_whore_knight",0x9999FF),
		(else_try),
			(display_message, "str_not_enough_gold",0x9999FF),
		(try_end),
		 (jump_to_menu, "mnu_town"), ]),
       ...
       ( "whore_4",mnf_scale_picture,
		...
		(try_begin),
			(ge, ":player_gold", 1000),
			(troop_remove_gold,"trp_player",1000),
			(call_script, "script_whore_knight",0),
			(store_random_in_range, reg1, 5,11),
			(party_add_members, "p_main_party", reg0, reg1),
			(display_message, "str_whore_knight",0x9999FF),
		(else_try),
       ...
       ( "whore_5",mnf_scale_picture,
          "her father is a lord",
          "none",
         [	(store_random_in_range, ":random", 0, 100),
			(try_begin),
				(lt,":random",33),
		 ...
		 ("whore_give_1500",
		 [],
		 "gevi her some money (cost 1500).",             
         [
		 (try_begin),
			(store_troop_gold,":player_gold", "trp_player"),
			(ge, ":player_gold", 1500),
			(troop_remove_gold,"trp_player",1500),
			(display_message, "str_whore_nobility",0x9999FF),
	  ...
	  ( "whore_7",mnf_scale_picture,
          "you find u are cooler.",
          "none",
         [	(store_random_in_range, ":random", 0, 100),
			(try_begin),
				(lt,":random",33),
         ...
         [  ("continue",[],"continue",             
            [
			(troop_raise_attribute, "trp_player",ca_charisma,1),
			(display_message, "str_whore_charisma",0x9999FF),
			(jump_to_menu, "mnu_town"), ]),
		]), 
...
## ZZ
      ("recruit_mercenary",
      [    
		(gt,"$temp_castle_troop",-1),
		(gt,"$temp_castle_num",-1),
		(str_store_troop_name,s10,"$temp_castle_troop"),
		(assign,reg10,"$temp_castle_num"),
		(store_troop_gold,":gold","trp_player"),
		(ge,":gold",300),		
	  ],
      "Recruit {reg10} {s10} (need 300)",
      [
	  		(party_get_free_companions_capacity, ":capacity", "p_main_party"),
			(try_begin),
				(ge,":capacity","$temp_castle_num"),
				(party_add_members,"p_main_party","$temp_castle_troop","$temp_castle_num"),
				(troop_remove_gold,"trp_player",300),
				(assign,"$temp_castle_troop",-1),
				(assign,"$temp_castle_num",-1),
				(display_message,"@They joined your party."),
			(else_try),
				(display_message,"@There is no capacity for them to join!",0xFF4433),
			(try_end),
      ]),
      ("station",[],"Station armys.",[(change_screen_exchange_members,1),]),
      ("wait",[],"Wait here for some time.",[(rest_for_hours_interactive, 24 * 7, 5, 0),(change_screen_return),]),	  
      ("leave",[],"Leave.",[(leave_encounter),(change_screen_return)]),
    ]
  ),

(
    "gate1",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ("world_other_side",[],"World_other_side.",
       [
       (party_get_position, pos1, "p_gate2"),
       (party_set_position, "p_main_party", pos1),
       (change_screen_return),
       ]),
...
(
    ...
    [
      ...
      ("world_other_side",[],"World_other_side.",
       [
       (party_get_position, pos1, "p_gate1"),
       (party_set_position, "p_main_party", pos1),
       (change_screen_return),
       ]),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon1_quit"),
			(assign, ":curr_scene", "scn_manor"),
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_spain_sergeant"),
			(set_visitor, 5, "trp_spain_sergeant"),
			(set_visitor, 6, "trp_spain_sergeant"),
			(set_visitor, 7, "trp_spain_sergeant"),
			(set_visitor, 8, "trp_spain_sergeant"),
			(set_visitor, 9, "trp_spain_sergeant"),
			(set_visitor, 10, "trp_spain_sergeant"),
			(set_visitor, 11, "trp_spain_sergeant"),
			(set_visitor, 12, "trp_spain_sergeant"),
			(set_visitor, 13, "trp_spain_sergeant"),
			(set_visitor, 14, "trp_spain_sergeant"),
			(set_visitor, 15, "trp_spain_sergeant"),
			(set_visitor, 16, "trp_spain_sergeant"),
			(set_visitor, 16, "trp_spain_sergeant"),
			(set_jump_mission,"mt_dungeon1"),
...
(
    "dungeon1_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_red_great_helmet","itm_volencia_king_armor","itm_red_great_helmet","itm_volencia_king_armor"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
		]),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon2_quit"),
			(assign, ":curr_scene", "scn_diaoyudao"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_wokou"),
			(set_visitor, 5, "trp_wokou"),
			(set_visitor, 6, "trp_japan_crossbowman"),
			(set_visitor, 7, "trp_wokou"),
			(set_visitor, 8, "trp_japan_footman"),
			(set_visitor, 9, "trp_wokou"),
			(set_visitor, 10, "trp_japan_crossbowman"),
			(set_visitor, 11, "trp_japan_footman"),
			(set_visitor, 12, "trp_wokou"),
			(set_visitor, 13, "trp_japan_footman"),
			(set_visitor, 14, "trp_wokou"),
			(set_visitor, 15, "trp_wokou"),
			(set_visitor, 16, "trp_japan_crossbowman1"),
			(set_visitor, 16, "trp_japan_crossbowman1"),
			(set_jump_mission,"mt_dungeon2"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon2_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_kabuto_inv","itm_yoroi1","itm_suneate","itm_kote_L"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon3_quit"),
			(assign, ":curr_scene", "scn_baimuda"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_spain_sergeant"),
			...
			(set_visitor, 6, "trp_spain_sergeant"),
			(set_visitor, 7, "trp_spain_sergeant"),
			(set_visitor, 8, "trp_spain_lancer"),
			(set_visitor, 9, "trp_spain_lancer"),
			(set_visitor, 10, "trp_spain_lancer"),
			(set_visitor, 11, "trp_spain_veteran_horse_archer"),
			(set_visitor, 12, "trp_spain_horse_archer"),
			(set_visitor, 13, "trp_spain_horse_archer"),
			(set_visitor, 14, "trp_spain_horse_archer"),
			(set_visitor, 15, "trp_spain_horse_archer"),
			(set_visitor, 16, "trp_spain_horse_archer"),
			(set_visitor, 16, "trp_spain_veteran_horse_archer"),
			(set_jump_mission,"mt_dungeon3"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon3_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_qsk","itm_qsj","itm_qsj","itm_hourglass_gauntlets_L"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon4_quit"),
			(assign, ":curr_scene", "scn_yamaxun"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_africa_sergeant"),
			(set_visitor, 5, "trp_africa_skirmisher"),
			(set_visitor, 6, "trp_africa_skirmisher"),
			(set_visitor, 7, "trp_africa_skirmisher"),
			(set_visitor, 8, "trp_sh_veteran_footman"),
			(set_visitor, 9, "trp_sh_veteran_footman"),
			(set_visitor, 10, "trp_africa_sergeant"),
			(set_visitor, 11, "trp_sh_veteran_footman"),
			(set_visitor, 12, "trp_sh_veteran_footman"),
			(set_visitor, 13, "trp_sh_veteran_footman"),
			(set_visitor, 14, "trp_sh_veteran_footman"),
			(set_visitor, 15, "trp_sh_mamluke"),
			(set_visitor, 16, "trp_sh_mamluke"),
			(set_visitor, 16, "trp_sh_mamluke"),
			(set_jump_mission,"mt_dungeon4"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon4_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_combed_morion_blued","itm_sarranid_veiled_helmet","itm_combed_morion_blued","itm_sarranid_veiled_helmet"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon5_quit"),
			(assign, ":curr_scene", "scn_jinzita"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_undead111"),
			(set_visitor, 5, "trp_undead111"),
			(set_visitor, 6, "trp_undead111"),
			(set_visitor, 7, "trp_undead111"),
			(set_visitor, 8, "trp_undead111"),
			(set_visitor, 9, "trp_undead111"),
			(set_visitor, 10, "trp_undead111"),
			(set_visitor, 11, "trp_undead111"),
			(set_visitor, 12, "trp_undead111"),
			(set_visitor, 13, "trp_undead111"),
			(set_visitor, 14, "trp_undead111"),
			(set_visitor, 15, "trp_undead111"),
			(set_visitor, 16, "trp_undead111"),
			(set_visitor, 16, "trp_undead111"),
			(set_jump_mission,"mt_dungeon5"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon5_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_sarranid_veiled_helmet","itm_khassakileather","itm_sarranid_veiled_helmet","itm_khassakileather"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon8_quit"),
			(assign, ":curr_scene", "scn_dongcang"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_dongcang"),
			(set_visitor, 5, "trp_dongcang"),
			(set_visitor, 6, "trp_dongcang"),
			(set_visitor, 7, "trp_dongcang"),
			(set_visitor, 8, "trp_dongcang"),
			(set_visitor, 9, "trp_dongcang"),
			(set_visitor, 10, "trp_dongcang"),
			(set_visitor, 11, "trp_dongcang"),
			(set_visitor, 12, "trp_dongcang"),
			(set_visitor, 13, "trp_dongcang"),
			(set_visitor, 14, "trp_dongcang"),
			(set_visitor, 15, "trp_dongcang"),
			(set_visitor, 16, "trp_dongcang"),
			(set_visitor, 16, "trp_dongcang"),
			(set_jump_mission,"mt_dungeon8"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon8_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_daming_helmet_20","itm_daming_armor_17","itm_daming_helmet_20","itm_daming_armor_17"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon9_quit"),
			(assign, ":curr_scene", "scn_skycity"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_hospital_sergeant"),
			(set_visitor, 5, "trp_hospital_sergeant"),
			(set_visitor, 6, "trp_sarranid_mamluke1"),
			(set_visitor, 7, "trp_sarranid_mamluke1"),
			(set_visitor, 8, "trp_hospital_sergeant"),
			(set_visitor, 9, "trp_hospital_lancer"),
			(set_visitor, 10, "trp_sarranid_mamluke1"),
			(set_visitor, 11, "trp_hospital_lancer"),
			(set_visitor, 12, "trp_hospital_lancer"),
			(set_visitor, 13, "trp_sarranid_mamluke1"),
			(set_visitor, 14, "trp_livonia_sergeant"),
			(set_visitor, 15, "trp_livonia_sergeant"),
			(set_visitor, 16, "trp_livonia_lancer"),
			(set_visitor, 16, "trp_livonia_lancer"),
			(set_jump_mission,"mt_dungeon9"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon9_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_turban_helmet_new","itm_heavy_yawshan","itm_turban_helmet_new","itm_heavy_yawshan"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon10_quit"),
			(assign, ":curr_scene", "scn_cangku"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_poland_lithuania_crossbowman"),
			(set_visitor, 5, "trp_poland_lithuania_crossbowman"),
			(set_visitor, 6, "trp_poland_lithuania_crossbowman"),
			(set_visitor, 7, "trp_poland_lithuania_crossbowman"),
			(set_visitor, 8, "trp_poland_lithuania_crossbowman"),
			(set_visitor, 9, "trp_poland_lithuania_veteran_horse_archer"),
			(set_visitor, 10, "trp_poland_lithuania_veteran_horse_archer"),
			(set_visitor, 11, "trp_poland_lithuania_horseman"),
			(set_visitor, 12, "trp_poland_lithuania_horseman"),
			(set_visitor, 13, "trp_poland_lithuania_horseman"),
			(set_visitor, 14, "trp_poland_lithuania_horseman"),
			(set_visitor, 15, "trp_poland_lithuania_veteran_horse_archer"),
			(set_visitor, 16, "trp_poland_lithuania_veteran_horse_archer"),
			(set_visitor, 16, "trp_poland_lithuania_veteran_horse_archer"),
			(set_jump_mission,"mt_dungeon10"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon10_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_husar_sholom","itm_pol_husar_yaguar","itm_husar_sholom","itm_pol_husar_yaguar"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
(
    ...
    [
      ...
      ("continue", [], "Continue...",
       [
			(assign, "$g_battle_result", 0), #add by windmelodie
			(assign, "$g_next_menu", "mnu_dungeon11_quit"),
			(assign, ":curr_scene", "scn_zhujuan"),##yifeng
                        (modify_visitors_at_site, ":curr_scene"),
			(reset_visitors),
			(set_visitor, 0, "trp_player"),
			(set_visitor, 4, "trp_swadian_infantry"),
			(set_visitor, 5, "trp_swadian_infantry"),
			(set_visitor, 6, "trp_swadian_footman"),
			(set_visitor, 7, "trp_swadian_footman"),
			(set_visitor, 8, "trp_swadian_footman"),
			(set_visitor, 9, "trp_swadian_man_at_arms"),
			(set_visitor, 10, "trp_swadian_man_at_arms"),
			(set_visitor, 11, "trp_swadian_man_at_arms"),
			(set_visitor, 12, "trp_swadian_man_at_arms"),
			(set_visitor, 13, "trp_swadian_knight"),
			(set_visitor, 14, "trp_swadian_knight"),
			(set_visitor, 15, "trp_swadian_knight"),
			(set_visitor, 16, "trp_swadian_knight"),
			(set_visitor, 16, "trp_swadian_knight"),
			(set_jump_mission,"mt_dungeon11"),
			(jump_to_scene,":curr_scene"),
			(scene_set_slot, ":curr_scene", slot_scene_visited, 1),
			(change_screen_mission),
		]),
...
(
    "dungeon11_quit",mnf_scale_picture,
    "YY",
    "none",
    [],
    [
      ...
      ("continue", [(neq, "$g_battle_result", -1)], "Continue...",
       [
#changed by windmelodie begin
			(call_script,"script_random_from_four","itm_toukui2","itm_zhongjia_4","itm_xie4","itm_zuoshou2_L"),
			(troop_add_item, "trp_player", reg0),
#changed by windmelodie end
			(str_store_string, s5, "@successd"),
			(jump_to_menu, "mnu_auto_return_to_map"),
...
#Windmelodie Outpost Changed begin
	("outpost", 0, "You pass across the barricades and enter the outpost. ^^ What do you want to do?", "none", [],
         [
			...
			(try_begin),
			 ...
			 (try_begin),
				...
				(party_set_slot, "$g_encountered_party", slot_outpost_level, 3),
				...
				#Windmelodie add
				(party_set_faction, "p_fort", "fac_player_supporters_faction"),
				(troop_get_slot, ":cur_banner", "trp_player", slot_troop_banner_scene_prop),
				(try_begin),
					(gt, ":cur_banner", 0),
					(val_sub, ":cur_banner", banner_scene_props_begin),
					(val_add, ":cur_banner", banner_map_icons_begin),
					(party_set_banner_icon,"p_fort", ":cur_banner"),
				(try_end),
				...
				(display_message, "@Outpost upgraded."),
				(call_script, "script_update_volunteer_troops_in_town_and_castle", "p_fort"),
				(party_set_slot,"p_fort",slot_center_troop_1,reg0),
				(party_set_slot, "p_fort", slot_town_prosperity,50),
				(change_screen_return),
			 (else_try),
		...
		("outpost_patrol_back", [(neg|party_slot_eq, "$g_encountered_party", slot_outpost_patrol, 0)], 
			"call back patrol",
		[(party_get_slot, ":outpost_patrol","$g_encountered_party",slot_outpost_patrol),
        (party_set_slot, ":outpost_patrol", slot_cattle_driven_by_player, 3),
        (party_set_ai_behavior, ":outpost_patrol", ai_bhvr_escort_party),
		(party_set_ai_object,":outpost_patrol", "$g_encountered_party"),
		 (change_screen_return),
		    ]),

			
		("reappoint_leader", [(party_get_slot, ":outpost_patrol","$g_encountered_party",slot_outpost_patrol),
		(neq, ":outpost_patrol", 0),
		(party_stack_get_troop_id, ":partol_troop",":outpost_patrol",0),
		(neq,":partol_troop","trp_fort_rider")], 
			"reappoint partrol leader",
		[(party_get_slot, ":outpost_patrol","$g_encountered_party",slot_outpost_patrol),
		(party_clear, "p_temp_party"),
		(party_add_members, "p_temp_party", "trp_fort_rider", 1),
		(call_script, "script_party_add_party", "p_temp_party",":outpost_patrol"),
		(party_clear, ":outpost_patrol"),
		(call_script, "script_party_add_party", ":outpost_patrol","p_temp_party"),
		(party_clear, "p_temp_party"),
		 (change_screen_return),
		    ]),
		...
		[(party_slot_eq, "$g_encountered_party", slot_outpost_patrol, 0),
		(party_slot_ge, "$g_encountered_party", slot_outpost_level, 2),], 
				"Enlist a patrol (3000 denars)",
	        [(store_troop_gold, ":gold_amount", "trp_player"),
		     (try_begin),
			    ...
			    (troop_remove_gold, "trp_player", 3000),
			    (set_spawn_radius, 2),
			    (spawn_around_party, "$g_encountered_party", "pt_kingdom_hero_party"),
			    (assign, ":outpost_patrol", reg0),
				(party_add_members, ":outpost_patrol", "trp_fort_rider", 1), #ADD TROOPS HERE
				(party_set_faction, ":outpost_patrol",fac_player_faction),
			    (party_set_flags, ":outpost_patrol", pf_default_behavior, 0),
				(party_set_aggressiveness, ":outpost_patrol", 10), # Very aggressive
				(party_set_courage, ":outpost_patrol", 10),        # Even more courageous
				(party_set_slot, ":outpost_patrol", slot_cattle_driven_by_player, 2), # I use this to dictate tehir behaviour
			    (party_set_ai_behavior, ":outpost_patrol", ai_bhvr_patrol_location),
			    (party_set_ai_object, ":outpost_patrol", "$g_encountered_party"),
				(party_get_position, pos1, "$g_encountered_party"),
				(party_set_ai_patrol_radius, ":outpost_patrol", 10),
				(party_set_ai_target_position, ":outpost_patrol", pos1),
				...
				(call_script, "script_cf_reinforce_patrol", ":outpost_patrol"),
				(call_script, "script_cf_reinforce_patrol", ":outpost_patrol"),
				(call_script, "script_cf_reinforce_patrol", ":outpost_patrol"),
				(faction_set_slot,"fac_mod_variables",slot_Mod_fort_patrol_reinforce,0),
				...
				(party_set_banner_icon, ":outpost_patrol", ":cur_faction_banner"),
			 (else_try),
			    (lt, ":gold_amount", 3000),
...
(
    ...
    [
     ...
     ("fort_station_troops",
      ...
      ("fort_rename", [(str_store_party_name, s1, "p_fort"),],
         "Rename this fort. (To your current name)",
         [
				(start_presentation, "prsnt_name_castle"),
          ]),
...
## ZZ Aid to develop changed by windmelodie begin 
        ("aid_to_develop",
        [ ],
        "Aid the center to develop",
        [(jump_to_menu,"mnu_aid_to_develop"),]),	
## ZZ Aid to develop changed by windmelodie end	  
...
#Windmelodie begin
	##mnu_enemy_offer_ransom_for_prisoner_asked
 (
    "enemy_offer_ransom_for_prisoner_asked",0,
    "the family of {s1} offers you a sum of {reg12} denars in silver if you are willing to sell him {s1}.",
    "none",
    [(call_script, "script_calculate_ransom_amount_for_troop", "$g_ransom_offer_troop_asked"),
     (assign, reg12, reg0),
	 (val_mul, reg12, 7),
	 (val_div, reg12, 10),
     (str_store_troop_name, s1, "$g_ransom_offer_troop_asked"),
     ],
    [
      ("ransom_accept_asked",[],"Very good.",
       [(troop_add_gold, "trp_player", reg12),
        (party_remove_prisoners, "$g_ransom_offer_party_asked", "$g_ransom_offer_troop_asked", 1),
        (call_script, "script_remove_troop_from_prison", "$g_ransom_offer_troop_asked"),
        (troop_set_slot, "$g_ransom_offer_troop_asked", slot_troop_leaded_party, 1),
        (assign, "$g_ransom_offer_troop_processing", 0),
        (change_screen_return),
        ]),
      ("ransom_reject_asked",[],"Reject the offer.",
       [
	    (call_script, "script_change_player_relation_with_troop", "$g_ransom_offer_troop", -4),
        (call_script, "script_change_player_honor", -1),
        (assign, "$g_ransom_offer_troop_processing", 0),
  ...
  ##mnu_enemy_offer_ransom_for_prisoner_asked_2, no longer there
 (
    "enemy_offer_ransom_for_prisoner_asked_2",0,
    "{s1} is nolonger at {s2}",
    "none",
    [(str_store_troop_name, s1, "$g_ransom_offer_troop_asked"),
     (str_store_party_name,s2,"$g_ransom_offer_party_asked"),
     ],
    [
      ("prisoner_nolonger",[],"Okay...",
       [(assign, "$g_ransom_offer_troop_processing", 0),
        (change_screen_return),
        ]),
     ]
...
#barrack added by windmelodie begin
    ("reinforcements_a",0,
    "here are {s20} and you need {reg20} to hire them",
    "none",
    [
	(store_faction_of_party, ":party_faction", "$current_town"),
	(faction_get_slot, ":party_template", ":party_faction", slot_faction_reinforcements_a),
	(party_clear, "p_temp_party"),
	(party_add_template, "p_temp_party", ":party_template"),
	(party_add_template, "p_temp_party", ":party_template"),
	(call_script, "script_cal_temp_party_price"),
	(str_store_string, s20, s1),
	(assign,reg20,reg0),
	],
    [  		
		("not_enough_relationship",
			[
			(neg|party_slot_ge, "$current_town", slot_center_player_relation, 5),
			(str_store_party_name, s2, "$current_town"),
			],
           "these warriors don't trust you",
			[(party_clear, "p_temp_party"),
			(jump_to_menu, "mnu_town"),	
			]
		),	
		...
		("not_enough_space",
			...
			(ge,":gold",":cost"),
			(party_get_free_companions_capacity, ":free_capacity", "p_main_party"), 
			(store_party_size_wo_prisoners,":capacity","p_temp_party"),
			(lt,":free_capacity",":capacity"),
			],
		...
		("hire_them",
           ...
           "hire them",
			...
			(party_set_slot,"$current_town",slot_center_troop_1,-1),
			(party_clear, "p_temp_party"),
			(jump_to_menu, "mnu_town"),	
			]
		),	
    ...
    [
	(store_faction_of_party, ":party_faction", "$current_town"),
	(faction_get_slot, ":party_template", ":party_faction", slot_faction_reinforcements_b),
	(party_clear, "p_temp_party"),
	(party_add_template, "p_temp_party", ":party_template"),
	(call_script, "script_cal_temp_party_price"),
	(str_store_string, s20, s1),
    ...
    [  		
		("not_enough_relationship",
			[
			(neg|party_slot_ge, "$current_town", slot_center_player_relation, 10),
			(str_store_party_name, s2, "$current_town"),
			],
           "these warriors don't trust you",
			[(party_clear, "p_temp_party"),
    ...
    [
	(store_faction_of_party, ":party_faction", "$current_town"),
	(faction_get_slot, ":party_template", ":party_faction", slot_faction_reinforcements_c),
	(party_clear, "p_temp_party"),
	(party_add_template, "p_temp_party", ":party_template"),
	(call_script, "script_cal_temp_party_price"),
	(str_store_string, s20, s1),
...
#barrack added by windmelodie end
 
## windmelodie patrol begin
    ("patrol", 0,
    "here you can manage your patrol",
    "none",
    [],
    [
	("outpost_patrol_back", [(neg|party_slot_eq, "$g_encountered_party", slot_outpost_patrol, 0)], 
...
#add by windmelodie 1.5.2	 
	("patrol_disband", [(neg|party_slot_eq, "$g_encountered_party", slot_outpost_patrol, 0)], 
			"call back patrol",
		[(party_get_slot, ":outpost_patrol","$g_encountered_party",slot_outpost_patrol),
		(party_set_slot,  "$g_encountered_party", slot_outpost_patrol, 0),
		(remove_party,":outpost_patrol"),
		 (change_screen_return),
	]),
#add by windmelodie 1.5.2 end		
...
## windmelodie patrol end
     ...
     ## HolyAce Ñ¡ÔñÄÄÒ»¹úÊ¿±ø ¿ªÊ¼
     ("which_country",0,
    "You will need to recruit?","none",[],
    [  		
	    ("Daming",[],"Daming",
           [
            (assign, "$which_country","fac_kingdom_7"),
            (jump_to_menu,"mnu_barrack1"),
              ],),
           
     ("Daqing",[],"Daqing",
           [
            (assign, "$which_country","fac_kingdom_20"),
            (jump_to_menu,"mnu_barrack1"),
              ],),	
              
     ("Menggu",[],"Menggu",
           [
            (assign, "$which_country","fac_kingdom_10"),
            (jump_to_menu,"mnu_barrack1"),
              ],),
           
     ("Riben",[],"Riben",
           [
            (assign, "$which_country","fac_kingdom_11"),
            (jump_to_menu,"mnu_barrack1"),
              ],),	
          ...
          ("forget_it",[],"Forget it.",
          [
               # (jump_to_menu,"mnu_zhai"),
			  (jump_to_menu,"mnu_camp"),
            ],),
          ],),
## HolyAce Ñ¡ÔñÄÄÒ»¹úÊ¿±ø ½áÊø
  
   ## HolyAce Ñ¡ÔñÕĞ½¨Öİ¶¼¶½Æï±ø»¹ÊÇ²½±ø ¿ªÊ¼
#     ("which",0,
#    "You will need to recruit?","none",[],
#    [  		
#
...
#            ],),
#          ],),
## HolyAce Ñ¡ÔñÕĞÄ¼Ö´ÆìÆï±ø»¹ÊÇ²½±ø ½áÊø
### HolyAce ÕĞÄ¼½¨Öİ¶¼¶½»ğÇ¹»¤ÎÀ ¿ªÊ¼
    ("barrack1",0, "How many are they do you want?", "none", [], [
	  ...
	  ("1",
       [         
       (try_begin),
         (eq,"$which_country" ,"fac_kingdom_20"),
    
    
(assign, "$remove_troop_1","trp_nvzhen_sergeant"),
(assign, "$remove_troop_2","trp_nvzhen_qingjiabushe"),
(assign, "$add_troop","trp_nvzhen_duduhuweihuoqiang"),

 		         (else_try),             
                        (eq,"$which_country" ,"fac_kingdom_7"),
                        
    (assign, "$remove_troop_1","trp_ming_dismounted_imperial_guard"),
(assign, "$remove_troop_2","trp_divine_engine_division_ming_musketeers"),
(assign, "$add_troop","trp_dongcang"),        

 		         (else_try),             
                        (eq,"$which_country" ,"fac_kingdom_10"),
                        
    (assign, "$remove_troop_1","trp_mongol_veteran_horse_archer"),
(assign, "$remove_troop_2","trp_mongol_lancer"),
(assign, "$add_troop","trp_mongol_lancer_guard"),      

 		         (else_try),             
                        (eq,"$which_country" ,"fac_kingdom_11"),
                        
    (assign, "$remove_troop_1","trp_japan_crossbowman"),
(assign, "$remove_troop_2","trp_japan_footman"),
(assign, "$add_troop","trp_japan_ninja"),      

        (try_end),	
          #   (party_set_slot, "$current_town", slot_center_troop_1, -1),
             #(store_troop_gold,":gold","trp_player"),
	     #(ge,":gold",2000),
		 		 (party_count_members_of_type, ":num_troop_1", "p_main_party", "$remove_troop_1"),
		 (party_count_members_of_type, ":num_troop_2", "p_main_party", "$remove_troop_2"),
         (ge,":num_troop_1",2),
         (ge,":num_troop_2",2),
        ],           
		"1 of them to join you.",
	   [
	         
                            
            (call_script, "script_Teshu Bingzhong Duihuan",1,2),
            (jump_to_menu,"mnu_barrack1"),
        ]),
...
(assign, "$add_troop","trp_japan_ninja"),      
        (try_end),	
         ...
         (ge,":num_troop_2",4),
             ...
             "2 of them to join you.",
	   [
#	   (assign, "$remove_troop_1","trp_nvzhen_sergeant"),
#(assign, "$remove_troop_2","trp_nvzhen_qingjiabushe"),
#(assign, "$add_troop","trp_nvzhen_duduhuweihuoqiang"),

            (call_script, "script_Teshu Bingzhong Duihuan",2,4),
            (jump_to_menu,"mnu_barrack1"),
        ]),
...
(assign, "$add_troop","trp_japan_ninja"),      
        ...
        (try_end),	 
         ...
         (ge,":num_troop_2",6),
             ...
             "3 of them to join you.",
	   [
	 #  (assign, "$remove_troop_1","trp_nvzhen_sergeant"),
    #   (assign, "$remove_troop_2","trp_nvzhen_qingjiabushe"),
   #    (assign, "$add_troop","trp_nvzhen_duduhuweihuoqiang"),
       
            (call_script, "script_Teshu Bingzhong Duihuan",3,6),
            (jump_to_menu,"mnu_barrack1"),
        ]), 			 
...
(assign, "$add_troop","trp_japan_ninja"),      
   ...
   #    (assign, "$add_troop","trp_nvzhen_duduhuweihuoqiang"),
	     ...
	     (party_count_members_of_type, ":num_troop_1", "p_main_party", "$remove_troop_1"),
		 (party_count_members_of_type, ":num_troop_2", "p_main_party", "$remove_troop_2"),
		 (store_div, ":div_num_troop_1", ":num_troop_1", 2),
		 (store_div, ":div_num_troop_2", ":num_troop_2", 2),
		 (val_min, ":div_num_troop_1", ":div_num_troop_2"),# Assigns <destination> := MAX (<destination>, <value>)
		 (store_mul, ":num_troop", ":div_num_troop_1", 2),
         (call_script, "script_Teshu Bingzhong Duihuan",":div_num_troop_1",":num_troop"),
         (jump_to_menu,"mnu_barrack1"),
        ]),
...
### HolyAce ÕĞÄ¼½¨Öİ¶¼¶½»ğÇ¹»¤ÎÀ ½áÊø
### HolyAce ÕĞÄ¼Ö´ÆìÆï±ø ¿ªÊ¼
#    ("barrack2",0,
#    "How many are they do you want?","none",[],
#    [  		
#	  ("join_bannerq_q_5",
#        [        
#          #   (party_set_slot, "$current_town", slot_center_troop_1, -1),
##	     (store_troop_gold,":gold","trp_player"),
#	     #(ge,":gold",3000),
#		# (main_party_has_troop, "trp_nvzhen_horse_archer"),
#	    # (main_party_has_troop, "trp_nvzhen_horse_archer_red"),
#		 (party_count_members_of_type, ":num_troop_1", "p_main_party", "trp_nvzhen_horse_archer"),
#		 (party_count_members_of_type, ":num_troop_2", "p_main_party", "trp_nvzhen_horse_archer_red"),
#         (ge,":num_troop_1",2),
#         (ge,":num_troop_2",2),
#        ],
#	    "1 of them to join you.",
#	   [
#	   	    (assign, "$remove_troop_1","trp_nvzhen_horse_archer"),
#            (assign, "$remove_troop_2","trp_nvzhen_horse_archer_red"),
#            (assign, "$add_troop","trp_nvzhen_duduhuwei"),
#            (call_script, "script_Teshu Bingzhong Duihuan",1,2),
#            (jump_to_menu,"mnu_barrack2"),
#        ]),
#          
...
#          ("forget_it",[],"Forget it.",
#         [
#                (jump_to_menu,"mnu_which"),
#            ]),			 
#	]),
### HolyAce ÕĞÄ¼Ö´ÆìÆï±ø ½áÊø
